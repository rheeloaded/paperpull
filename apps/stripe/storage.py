"""What makes the Stripe app different, everything else is paperpull_core.
...
declaration of Stripe's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair Stripe's *page* behavior, edit `stripe_site.py` instead.
"""
...
# One row per downloaded Stripe document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    kind=DOCUMENT,
    folders=[
        # A fee invoice classifies as a statement, it is what Stripe bills.
        Folder("statements", "Fee Invoices"),
