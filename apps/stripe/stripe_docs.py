"""Stripe fee invoice and tax form downloader (local, supervised).

Opens the Invoice history and My documents pages of the signed-in Stripe
Dashboard, keeps the lists those pages receive, and saves each fee invoice
and tax form PDF from its own link. Nothing on the page is clicked. The
site layer, stripe_site.py, holds every fact about stripe.com.

Usage:
    python stripe_docs.py --login       verify connection to your browser
    python stripe_docs.py --discover    list available documents
    python stripe_docs.py --pilot       download the 5 newest, then stop
    python stripe_docs.py --all         download everything in scope
    python stripe_docs.py --resume      continue an interrupted run
    python stripe_docs.py --verify      re-validate every saved PDF
    python stripe_docs.py --diagnose    dump page structure (no downloads)
    python stripe_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the two document lists and saves the
PDFs Stripe already generated. It never pays out, refunds, charges,
creates anything, regenerates an invoice, answers a tax ID request, or
changes any account setting. Everything stays on this machine; nothing is sent to any
external service.
"""
...
log = logging.getLogger("stripe_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. Stripe gives every invoice and tax form its own
        id, so that is the key. Category, date, title and account are the
        fallback for a record written without one."""
        if self.document_id:
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Stripe tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Stripe appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9275")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. The Dashboard runs hCaptcha at
        # sign-in and is happiest in a real browser.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Stripe browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Stripe shows a signed-out page.")
            print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
        elif ok:
            print("Success: connected and signed in to the Stripe Dashboard.")
            print("Keep that browser window OPEN, then run:  paperpull stripe pilot")
        else:
            print("Connected, but I could not open the Stripe Dashboard.")
            print("Open dashboard.stripe.com in that browser yourself, then run --diagnose.")
        self.close()
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        ...
        # A Stripe sign-in reads one account at a time, so this is empty.
        ...
        doc = Document(title=title, category=category, summary=full_summary,
                       ...
                       document_id=getattr(r, "document_id", "") or "")
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Be on the Dashboard, then open each list page and keep its answer.
        ...
        self.discovery.save()
        log.info("Invoice history and My documents: %d documents, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
        ...
        except site.NeedsPerson as e:
            # Stripe wants something only the account holder may give,
            # such as the tax ID before a 1099. Never answered here.
            print(f"\n  !! {e}. Download it yourself in the Dashboard,")
            print("     Settings, Compliance and documents.")
            ...
            print("\n  !! Stripe answered with a sign-in page instead of a document.")
            print("     Your session has expired. Sign in again in the open")
            print("     browser, then run:  paperpull stripe resume")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Stripe documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Stripe', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        """Survey each list page and the shape of what it received, and
        ...
        info = {"timestamp": now_iso(),
                "note": "What the Stripe invoice and tax form lists look like, "
        ...
        print("then attach it to the Stripe issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Stripe document downloader (read-only)")
    ...
    ap.add_argument("--type", help='Statement (fee invoices) or "Tax Document"')
