"""What makes the Chase app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Chase's own
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Out of scope for this app (see chase_site.py), so the folder is
        ...
        # Chase issues no insurance documents. The route stays so a
    ...
    csv_files=[
        CsvSpec("document_index_csv", "Chase Document Index.csv", DOCUMENT_INDEX_COLUMNS),
    ],
    config_defaults={
        ...
        "pilot_count": 5,
        # Chase posts one statement per REGISTRATION (the name(s) an account is
    ...
    base_url="https://secure.chase.com/",
