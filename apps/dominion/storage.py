"""What makes the Dominion Energy app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Dominion Energy's own
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # A utility issues no tax forms. The route stays so a surprise one is
        # still filed rather than dropped, but the folder is made on demand.
    ...
    csv_files=[
        CsvSpec("document_index_csv", "Dominion Document Index.csv", DOCUMENT_INDEX_COLUMNS),
    ],
    config_defaults={
    ...
    base_url="https://www.dominionenergy.com/",
