...
def test_unique_path_no_collision(tmp_path):
    p = unique_path(tmp_path, "2026-07-19 Dominion Clothing Receipt.pdf")
    assert p.name == "2026-07-19 Dominion Clothing Receipt.pdf"


def test_unique_path_numbering(tmp_path):
    (tmp_path / "2026-07-19 Dominion Clothing Receipt.pdf").write_bytes(b"x")
    p2 = unique_path(tmp_path, "2026-07-19 Dominion Clothing Receipt.pdf")
    assert p2.name == "2026-07-19 Dominion Clothing Receipt (2).pdf"
    p2.write_bytes(b"x")
    p3 = unique_path(tmp_path, "2026-07-19 Dominion Clothing Receipt.pdf")
    assert p3.name == "2026-07-19 Dominion Clothing Receipt (3).pdf"


def test_unique_path_case_insensitive(tmp_path):
    (tmp_path / "2026-07-19 Dominion CLOTHING RECEIPT.PDF").write_bytes(b"x")
    p = unique_path(tmp_path, "2026-07-19 Dominion Clothing Receipt.pdf")
    assert p.name == "2026-07-19 Dominion Clothing Receipt (2).pdf"
