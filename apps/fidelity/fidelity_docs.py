"""Fidelity Investments statement and tax-form downloader (local, supervised).

Usage:
    python fidelity_docs.py --login       verify connection to your browser
    python fidelity_docs.py --discover    list available documents
    python fidelity_docs.py --pilot       download the 5 newest, then stop
    python fidelity_docs.py --all         download everything in scope
    python fidelity_docs.py --resume      continue an interrupted run
    python fidelity_docs.py --verify      re-validate every saved PDF
    python fidelity_docs.py --diagnose    dump page structure (no downloads)
    python fidelity_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the Document Access Hub and downloads
PDFs that Fidelity already generated. It never moves money between funds, changes
...
Mapped 2026-09-18. See the STATUS block in fidelity_site.py for the API, the
...
log = logging.getLogger("fidelity_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's signed-in Fidelity tab; a fresh one is unauthenticated.
            # Matched by parsed host, not by substring: "fidelity.com" in the URL
            # would also accept nottsp.com.example, and would have picked an
            # unrelated tab as the work page when no Fidelity tab was open.
            live = [p for p in ctx.pages if not p.is_closed()]
            mine = [p for p in live if site.is_safe_url(p.url or "")]
            if mine:
                self._work_page = mine[0]
            else:
                log.warning("No Fidelity tab is open. Sign in with login.bat and "
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Fidelity appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        name = browser_launcher.open_signin_browser(profile, port, url,
            prefer_real=True,   # Fidelity's bot management turns a bare Chromium away
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Fidelity browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Fidelity shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has Fidelity documents from 2023 and earlier).
    ...
    def _record_statement_doc(self, d: dict, occurrence: int = 0) -> int:
        ...
        if d.get("category"):
            ...
            category = d["category"]
            if category in (storage.CONFIRM, storage.STATEMENT) and summary in ("", "Other Document"):
                summary = "Trade Confirmation" if category == storage.CONFIRM else "Statement"
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.ensure_statements(page):
            ...
            if not site.ensure_statements(page):
                print("Could not reach the Document Access Hub. Sign in in the browser (the tab")
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            print("\n  !! Fidelity answered with a sign-in error instead of a document.")
    ...
    def _write_row(self, doc: Document, status: str, processing: str):
        ...
        self.index_csv.append_rows([{
            ...
            # No URL exists for a Fidelity document: it is fetched by a
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Fidelity documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Fidelity', text=text, extra=extra)
        if not path:
            return
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Fidelity Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Fidelity document downloader (read-only)")
