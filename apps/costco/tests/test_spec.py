"""The facts that must not drift once somebody has an archive.

The site layer of this app is nearly all guesswork and is meant to be
rewritten from a tester's recording. None of that is true of these. A
renamed CSV orphans the index, a moved folder hides the documents, and a
different provider string renames every new PDF, so once the first
Costco receipt lands on somebody's disk these are fixed.
"""
import sys
...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-01-02", "thing", "Statement") ==         "2026-01-02 Costco Thing Statement.pdf"


def test_csv_filenames_are_unchanged(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.order_history_csv.name == "Costco Order History.csv"
    assert paths.receipt_index_csv.name == "Costco Receipt Index.csv"
...
def test_precreated_folders_are_unchanged(tmp_path):
    ...
    assert made == ['Backups', 'Diagnostics', 'In-Warehouse', 'Logs',
                    'Manual Review', 'Online']


def test_every_declared_route_resolves(tmp_path):
    """The route key is the core's own constant and the folder is the
    word Costco uses, so these two differ on purpose and the mapping is
    the thing that has to hold."""
    paths = storage.Paths(tmp_path)
    for key in storage.SPEC.routes:
        assert paths.folder_for(key).is_dir()
    assert paths.folder_for("In-Store").name == "In-Warehouse"
