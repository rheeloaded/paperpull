"""What makes the Costco app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Costco's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair Costco's *page* behavior, edit `costco_site.py` instead.
"""
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Costco's own word for a purchase made at a warehouse. The route
        # key stays IN_STORE because that is the core's constant, and the
        # folder a person opens says what Costco says.
        Folder("instore", "In-Warehouse"),
        *INFRASTRUCTURE_FOLDERS,
    ],
    routes={
        "Online": "online",
    ...
    csv_files=[
        CsvSpec("order_history_csv", "Costco Order History.csv", ORDER_HISTORY_COLUMNS),
        CsvSpec("receipt_index_csv", "Costco Receipt Index.csv", RECEIPT_INDEX_COLUMNS),
    ],
    config_defaults={
        "pilot_online": 5,
        "pilot_instore": 3,
