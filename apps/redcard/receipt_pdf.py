...
log = logging.getLogger("redcard_docs.pdf")
...
# the receipts column on pages without stable ids/landmarks (Target Circle Card's
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://mytargetcirclecard.target.com/">',
...
def is_zip(path: Path) -> bool:
    """Some Target Circle Card tax forms (e.g. 1099-R) download as a ZIP holding the
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Target Circle Card, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Target Circle Card/order details",
...
def expected_tokens_for(purchase) -> list:
    ...
    tokens = ["target circle card", "targetcirclecard", "redcard"]
