...
def test_zip_detection_and_extraction(tmp_path):
    """Some issuers deliver a tax form as a ZIP holding the PDF; it must be
    detected and unpacked, not saved as a broken 'PDF'."""
    import zipfile
    ...
    assert is_zip(zpath)
    out = tmp_path / "2025-12-31 Target Circle Card 1099-INT Tax Form.pdf"
    zpath.replace(out)
...
def test_image_based_pdf_not_rejected_for_no_text(tmp_path):
    ...
    p = tmp_path / "img.pdf"
    make_pdf(p)
    r = validate_pdf(p, min_bytes=1000, expect_tokens=["target circle card", "12345"])
    assert r.ok, r.reason
