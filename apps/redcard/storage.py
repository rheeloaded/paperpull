"""Filesystem, filename, CSV, and progress storage for the Target Circle Card receipt downloader.
...
DEFAULT_OUTPUT_DIR = Path(r"C:\Users\YOU\Downloads\Target Circle Card Statements")
...
# One row per downloaded Target Circle Card document. Deliberately records NO
# account numbers, SSN, balances, or other sensitive values from inside the
# documents - only what is needed to find and verify a file locally.
...
def load_config(path: Optional[Path] = None) -> dict:
    ...
    cfg.setdefault("profile_dir", str(PROJECT_DIR / "redcard-browser-profile"))
    cfg.setdefault("owner", "")               # account holder's name
    cfg.setdefault("owner_in_filename", False)  # also prefix PDFs with the name
    return cfg
...
def ensure_owner(config: dict, config_path: Path) -> dict:
    """Make sure the account holder's name is set. If it's blank and we're on an
    interactive console, ask once and save it back to the config file so every
    document this account downloads is associated with that person. In a
    non-interactive run (e.g. the GUI) it's left blank rather than blocking."""
    if config.get("owner"):
        return config
...
class Paths:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.year_end_summaries = self.root / "Year-End Summaries"
        self.tax_documents = self.root / "Tax Documents"
        self.insurance_documents = self.root / "Insurance Documents"
        self.other_documents = self.root / "Other Documents"
        self.document_index_csv = self.root / "Target Circle Card Document Index.csv"
        self.order_history_csv = self.root / "Target Circle Card Order History.csv"
        self.receipt_index_csv = self.root / "Target Circle Card Receipt Index.csv"
    ...
    def all_dirs(self) -> List[Path]:
        ...
        return [self.root, self.statements, self.year_end_summaries,
                self.tax_documents, self.manual_review, self.logs,
    ...
    def folder_for(self, category: str, document_type: str = "") -> Path:
        ...
        if category == "Insurance Document":
            return self._ready(self.insurance_documents)
        if category == "Statement":
            return self._ready(self.statements)
        if category == "Year-End Summary":
            return self._ready(self.year_end_summaries)
...
def build_pdf_filename(purchase_date: str, summary: str,
    ...
    """YYYY-MM-DD [Owner ]Target Circle Card <Summary> <Receipt|Invoice>[ (i of n)].pdf"""
    ...
    base = f"{date} {who}Target Circle Card {summary} {document_type}"
