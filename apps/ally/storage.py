"""What makes the Ally app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Ally's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair Ally's *page* behaviour, edit `ally_site.py` instead.
"""
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Ally Bank issues no insurance documents. The route stays so a
    ...
    csv_files=[
        CsvSpec("document_index_csv", "Ally Document Index.csv", DOCUMENT_INDEX_COLUMNS),
    ],
    config_defaults={
        ...
        "pilot_count": 5,
        # Ally posts one statement per REGISTRATION (the name(s) an account is
