...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            ...
            dom = [p for p in live if "verizon.com" in (p.url or "")]
    ...
    def cmd_open_browser(self):
        ...
        # Verizon's WAF blocks the Playwright Chromium build (its user-agent and
        # fingerprint read as automation). A real, branded browser passes, so
        # prefer Microsoft Edge, then Google Chrome, then the bundled Chromium.
        ...
        print("Sign in, open your bill history, keep the window OPEN, then run the pilot.")
