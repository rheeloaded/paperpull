"""What makes the State Farm app different, everything else is paperpull_core.
...
declaration of State Farm's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair State Farm's *page* behavior, edit `statefarm_site.py` instead.
"""
...
# One row per downloaded State Farm document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Made only if a title ever routes there, never left sitting empty.
        Folder("tax_documents", "Tax Documents", precreate=False),
        # ID cards, declarations and policy documents.
    ...
    config_defaults={
        ...
        "pilot_count": 5,
        "document_types": ["Statement", "Insurance Document"],
    },
