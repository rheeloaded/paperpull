...
def _years_from(page, url: str, first_year: int) -> List[dict]:
    ...
    for year in range(this_year, this_year - YEARS_BACK - 1, -1):
        ...
        if target == url or not is_safe_url(target):
            break
        try:
            body = page.evaluate(_FETCH_JSON, target)
        except Exception as e:
...
def collect_download_docs(page) -> List[RawDoc]:
    ...
    if urls:
        ...
        found.extend(_years_from(page, urls[0], int(m.group(1)) if m else -1))
    for d in found:
        key = (d["date"], d["title"], d["hint"])
        if key in seen:
            continue
        seen.add(key)
...
def download_bill(page, dl_dir, iso_date: str, out_path, title: str = "",
                  trace: Optional[list] = None, hint: str = "") -> bool:
        ...
        if is_safe_url(target):
            try:
                ...
                if data[:5] == b"%PDF-":
                    ...
                    return True
                if trace is not None:
                    trace.append({"note": "filePathUrl did not answer with a PDF", "url": redact(target)[:160]})
            except Exception as e:
                if trace is not None:
