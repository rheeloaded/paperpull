...
statefarm_site.py, which carries every guess about statefarm.com and says which ones
...
Usage:
    python statefarm_docs.py --login       verify connection to your browser
    python statefarm_docs.py --discover    list available documents
    python statefarm_docs.py --pilot       download the 5 newest, then stop
    python statefarm_docs.py --all         download everything in scope
    python statefarm_docs.py --resume      continue an interrupted run
    python statefarm_docs.py --verify      re-validate every saved PDF
    python statefarm_docs.py --diagnose    dump page structure (no downloads)
    python statefarm_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the documents page and saves the PDFs State Farm
...
log = logging.getLogger("statefarm_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in State Farm tab.
        ...
        self._dl_dir = Path(self.config["output_dir"]) / ".statefarm-downloads"
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! State Farm appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9261")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first, since statefarm.com's sign-in is happiest in a real browser.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in State Farm browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but State Farm shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a State Farm page.
        Returns 1 if new."""
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available State Farm documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='State Farm', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        print("then attach it to the State Farm issue on GitHub. No screenshot was taken.")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "State Farm Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised State Farm document downloader (read-only)")
