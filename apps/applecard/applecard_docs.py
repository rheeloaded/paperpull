"""Apple Card and Savings statement and tax form downloader (local, supervised). UNVERIFIED, see applecard_site.py.
...
one every other statement app runs on, except that discovery visits three
sections rather than one page, the card's statements, the Savings
statements and wherever the tax forms are. What has not been proven is the
site layer, applecard_site.py, which carries every guess about
card.apple.com and says which ones most need a recording. Run --record
first and attach the file to the GitHub issue. It records no keystroke,
takes no screenshot and masks digit runs.

Usage:
    python applecard_docs.py --login       verify connection to your browser
    python applecard_docs.py --discover    list available documents
    python applecard_docs.py --pilot       download the 5 newest, then stop
    python applecard_docs.py --all         download everything in scope
    python applecard_docs.py --resume      continue an interrupted run
    python applecard_docs.py --verify      re-validate every saved PDF
    python applecard_docs.py --diagnose    dump page structure (no downloads)
    python applecard_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the statement pages and saves the PDFs Apple
already generated. It never pays, transfers, moves money between the card
and Savings, withdraws, touches Daily Cash, disputes, opens or closes
anything, or changes any account setting. Everything stays on this machine; nothing is
...
log = logging.getLogger("applecard_docs")
...
class Document:
    """One Apple Card or Savings document."""
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in card.apple.com tab.
        ...
        self._dl_dir = Path(self.config["output_dir"]) / ".applecard-downloads"
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Apple Card appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9270")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. An Apple Account sign-in is happiest in
        # the browser already on the machine, with its passkeys and password
        # manager, and the code it sends goes to the person's own devices.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Apple Card browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but card.apple.com shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a card.apple.com page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # The card's statements, the Savings statements and the tax forms
        # are three sections. Each is opened in turn and every control that
        # fetches one document is read. An account with no Savings simply
        # has no second section, which is not a failure.
        ...
        for kind in site.KINDS:
            ...
            n_kind = 0
            for r in docs:
                n_kind += self._record_rawdoc(r, page.url or site.BILLING_URL)
            n_new += n_kind
            self.discovery.save()
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        # The site layer opens the section this document's title names, then
        # finds and fetches it.
        trace: list = []
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Apple Card and Savings documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Apple Card', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "Apple Card app built without an account. This survey is what "
        ...
        try:
            ...
            # Each section in turn, with how the page was reached and what
            # was read there as counts, so a repair can see which of the
            # three guesses held.
            found_docs = []
            info["sections"] = {}
            for kind in site.KINDS:
                steps: list = []
                ok = site.goto_section(page, kind, steps)
                got = site.collect_download_docs(page, kind, steps) if ok else []
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Apple Card Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Apple Card and Savings document downloader (read-only)")
