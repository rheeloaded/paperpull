"""What makes the FedEx app different, everything else is paperpull_core.
...
declaration of FedEx's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair FedEx's *page* behavior, edit `fedex_site.py` instead.
"""
...
# One row per downloaded FedEx document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    kind=DOCUMENT,
    folders=[
        # A shipping invoice classifies as a statement, it is what FedEx bills.
