"""FedEx Billing Online invoice downloader (local, supervised). UNTESTED.

Opens the FedEx Billing Online invoices page, keeps the answers the page
receives, and saves each shipping invoice PDF from its own link. Nothing
on the page is clicked. The site layer, fedex_site.py, holds every fact
about fedex.com.

Usage:
    python fedex_docs.py --login       verify connection to your browser
    python fedex_docs.py --discover    list available documents
    python fedex_docs.py --pilot       download the 5 newest, then stop
    python fedex_docs.py --all         download everything in scope
    python fedex_docs.py --resume      continue an interrupted run
    python fedex_docs.py --verify      re-validate every saved PDF
    python fedex_docs.py --diagnose    dump page structure (no downloads)
    python fedex_docs.py --dry-run     plan filenames, save nothing
...
PDFs FedEx already generated. It never pays out, refunds, charges,
connects an account, disputes a charge, or
...
log = logging.getLogger("fedex_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. FedEx gives every invoice its own
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in FedEx tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! FedEx appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9278")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. fedex.com runs bot protection and
        # is happiest in a real browser.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in FedEx browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but FedEx shows a signed-out page.")
            print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
        elif ok:
            print("Success: connected and signed in to fedex.com.")
            print("Keep that browser window OPEN, then run:  paperpull fedex pilot")
        else:
            print("Connected, but I could not open fedex.com.")
            print("Open fedex.com in that browser yourself, then run --diagnose.")
        self.close()
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        ...
        # A FedEx sign-in reads one account at a time, so this is empty.
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Be on fedex.com, then open Billing Online and keep what it receives.
        ...
        try:
            ...
            # The shipping account is not connected to Billing Online. The
            # form that connects it is the account holder's to fill in.
            print(f"\n!! {e}.")
            print("   Open FedEx Billing Online in the browser and connect your")
            print("   shipping account there yourself, then run Discover again.")
            raise SystemExit(0)
        for r in docs:
            n_new += self._record_rawdoc(r, site.BILLING_URL)
        self.discovery.save()
        log.info("Billing Online: %d invoices, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            print("\n  !! FedEx answered with a sign-in page instead of a document.")
            print("     Your session has expired. Sign in again in the open")
            print("     browser, then run:  paperpull fedex resume")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available FedEx documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='FedEx', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(),
                "note": "What FedEx Billing Online's answers look like, "
        ...
        print("then attach it to the FedEx issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised FedEx document downloader (read-only)")
    ...
    ap.add_argument("--type", help="Statement (shipping invoices, the only kind)")
