"""SBA classification, the guard, and the survey's promises.
...
def test_titles_classify_the_way_the_filenames_need():
    for title, summary, cat in [('Monthly Statement - August 31, 2026', 'Monthly Statement', 'STATEMENT'), ('EIDL loan statement', 'Loan Statement', 'STATEMENT'), ('Loan Statement - July 2026', 'Loan Statement', 'STATEMENT'), ('1098', '1098 Tax Form', 'TAX'), ('Tax Document - 2025', 'Tax Document', 'TAX')]:
...
def test_the_noise_is_skipped():
    for t in ['Payment confirmation', 'Loan agreement', 'Promissory note', 'Hardship application', 'FAQ']:
...
def test_filename_shape():
    ...
    d, summ, kind, want = ('2026-08-31', 'Loan Statement', 'Statement', '2026-08-31 SBA Loan Statement Statement.pdf')
    assert build_pdf_filename(d, summ, kind) == want


def test_every_control_that_moves_money_or_changes_anything_is_refused():
    for label in ['Make a payment', 'Pay now', 'Schedule payment', 'Set up autopay', 'Recurring payment', 'Request hardship', 'Apply for deferment', 'Forgiveness application', 'Submit', 'Upload documents', 'Request payoff', 'Update bank account', 'Change password', 'Edit profile', 'Manage', 'Cancel', 'Confirm', 'Save Changes', 'Turn off', 'Chat with us', 'Certify', 'Appeal']:
        assert not site.is_safe_control(label), label


def test_the_controls_that_fetch_a_document_are_allowed():
    for label in ['View statement', 'Download statement', 'Statement PDF', '1098', 'Tax documents', 'View', 'Download', 'See more statements', 'Show older statements', 'Print']:
...
def test_the_survey_masks_numbers_and_takes_no_screenshot():
    ...
    docs_src = (Path(site.__file__).parent / "sba_docs.py").read_text(encoding="utf-8")
...
def test_the_survey_follows_only_documents_links():
    for text in ['Statements', 'My loans', 'Loan details', 'Tax documents', 'View statements']:
        assert site.SURVEY_LINK_RE.match(text), text
    for text in ['Make a payment', 'Apply', 'Home', 'Help']:
        assert not site.SURVEY_LINK_RE.match(text), text


def test_only_the_providers_own_hosts():
    for u in ['https://lending.sba.gov/statements', 'https://www.sba.gov/x', 'https://login.sba.gov/x']:
        assert site.is_safe_url(u), u
    for u in ['https://sba.gov.evil.test/s.pdf', 'http://lending.sba.gov/s.pdf', 'https://user@sba.gov/s.pdf', 'https://login.gov/x']:
