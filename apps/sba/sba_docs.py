"""SBA loan statement downloader (local, supervised). UNVERIFIED, see sba_site.py.

This app was written without an SBA account so that someone who holds one
...
sba_site.py, which carries every guess about sba.gov and says which ones
...
Usage:
    python sba_docs.py --login       verify connection to your browser
    python sba_docs.py --discover    list available documents
    python sba_docs.py --pilot       download the 5 newest, then stop
    python sba_docs.py --all         download everything in scope
    python sba_docs.py --resume      continue an interrupted run
    python sba_docs.py --verify      re-validate every saved PDF
    python sba_docs.py --diagnose    dump page structure (no downloads)
    python sba_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the statements area and saves the PDFs SBA
already generated. It never makes or schedules a payment,
applies or asks for anything, or changes any account setting. Everything stays on this machine; nothing is
...
log = logging.getLogger("sba_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in SBA tab.
        ...
        self._dl_dir = Path(self.config["output_dir"]) / ".sba-downloads"
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! SBA appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9254")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first, which is also what login.gov and the
        # portal's own sign-in expect. Any Chromium-based browser will do.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in SBA browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but SBA shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from an SBA page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Open the statements area and read the date from every control that
        ...
        self.discovery.save()
        log.info("statements area: %d documents, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        # Open the statements area and let the site layer find and fetch this one.
        if not site.goto_documents(page):
            self.check_session(page)
            site.goto_documents(page)
        trace: list = []
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available SBA documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='SBA', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        """Survey the statements area and write a file a tester can attach to
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "SBA app built without an account. This survey is what "
        ...
        print("then attach it to the SBA issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised SBA document downloader (read-only)")
