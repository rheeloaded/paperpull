"""What makes the SBA app different, everything else is paperpull_core.
...
declaration of SBA's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair SBA's *page* behavior, edit `sba_site.py` instead.
"""
...
# One row per downloaded SBA document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # A loan servicer issues a 1098 every year, so the folder is made up
        # front.
        Folder("tax_documents", "Tax Documents"),
        # Reachable if "Insurance Document" is added to document_types, but
        # SBA does not issue them, so it is created only if one ever
