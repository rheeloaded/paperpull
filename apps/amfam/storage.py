"""What makes the American Family app different, everything else is paperpull_core.
...
declaration of American Family's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair American Family's *page* behavior, edit `amfam_site.py` instead.
"""
...
# One row per downloaded American Family document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Policy documents, declarations pages, ID cards, renewal notices.
        Folder("insurance_documents", "Insurance Documents"),
        # An insurer rarely issues tax forms, so the folder waits for one.
    ...
    base_url="https://myaccount.amfam.com/",
