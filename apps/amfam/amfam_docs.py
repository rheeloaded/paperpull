"""American Family statement and tax document downloader (local, supervised). UNVERIFIED, see amfam_site.py.

This app was written without a American Family account so that someone who holds one
...
amfam_site.py, which carries every guess about amfam.com and says which ones
...
Usage:
    python amfam_docs.py --login       verify connection to your browser
    python amfam_docs.py --discover    list available documents
    python amfam_docs.py --pilot       download the 5 newest, then stop
    python amfam_docs.py --all         download everything in scope
    python amfam_docs.py --resume      continue an interrupted run
    python amfam_docs.py --verify      re-validate every saved PDF
    python amfam_docs.py --diagnose    dump page structure (no downloads)
    python amfam_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the Statements & Documents page and saves the PDFs American Family
...
log = logging.getLogger("amfam_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in American Family tab.
        ...
        self._dl_dir = Path(self.config["output_dir"]) / ".amfam-downloads"
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! American Family appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9267")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. An insurer's sign-in is happiest in the browser already on the
        # machine, with its passkeys and password manager.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in American Family browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but American Family shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a American Family page.
        Returns 1 if new."""
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available American Family documents ({scope}).")
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "American Family app built without an account. This survey is what "
        ...
        print("then attach it to the American Family issue on GitHub. No screenshot was taken.")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "American Family Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised American Family document downloader (read-only)")
