"""Locks in the facts that must not drift for the Anthem app.

A renamed CSV orphans the index, a moved folder hides the documents, and a
different provider string renames every new PDF. These pin them.
"""
import sys
...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-01-02", "thing", "Statement") == \
        "2026-01-02 Anthem Thing Statement.pdf"


def test_csv_filenames_are_unchanged(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.document_index_csv.name == "Anthem Document Index.csv"
...
def test_precreated_folders_are_unchanged(tmp_path):
    ...
    # EOBs is the only document folder pre-created; Tax Documents and Other
    # Documents appear only when something actually routes there.
    assert made == ['Backups', 'Diagnostics', 'EOBs', 'Logs', 'Manual Review']
