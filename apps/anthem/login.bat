@echo off
cd /d "%~dp0"
rem  login.bat          -> your account (config.json, port 9241)
rem  login.bat spouse   -> config.spouse.json (own folders, profile, port)
if "%~1"=="" (set "CFG=") else (set "CFG=--config config.%~1.json")
echo ============================================================
echo  Anthem Documents - sign in
echo ============================================================
if not "%~1"=="" echo Account: %~1
echo.
echo A normal Chromium window will open. Then:
echo   1. Sign in to Anthem (do all the 2FA / verification yourself).
...
echo.
.venv\Scripts\python.exe anthem_docs.py --open-browser %CFG%
echo.
pause
