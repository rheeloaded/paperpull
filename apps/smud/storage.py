"""What makes the SMUD app different, everything else is paperpull_core.
...
declaration of SMUD's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair SMUD's *page* behavior, edit `smud_site.py` instead.
"""
...
# One row per downloaded SMUD document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # SMUD does not issue them, so it is created only if one ever
