"""SMUD statement and tax document downloader (local, supervised). UNVERIFIED, see smud_site.py.

This app was written without a SMUD account so that someone who holds one
...
smud_site.py, which carries every guess about smud.org and says which ones
...
Usage:
    python smud_docs.py --login       verify connection to your browser
    python smud_docs.py --discover    list available documents
    python smud_docs.py --pilot       download the 5 newest, then stop
    python smud_docs.py --all         download everything in scope
    python smud_docs.py --resume      continue an interrupted run
    python smud_docs.py --verify      re-validate every saved PDF
    python smud_docs.py --diagnose    dump page structure (no downloads)
    python smud_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the documents page and saves the PDFs SMUD
...
log = logging.getLogger("smud_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in SMUD tab.
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9259")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first, since a utility portal is happiest in a real browser.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in SMUD browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but SMUD shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a SMUD page.
        Returns 1 if new."""
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available SMUD documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='SMUD', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "SMUD app built without an account. This survey is what "
        ...
        print("then attach it to the SMUD issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised SMUD document downloader (read-only)")
