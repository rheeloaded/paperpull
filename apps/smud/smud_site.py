"""ALL smud.org selectors, URLs, and page behavior live here.
...
signed-in site. On a first run it is deliberately cautious:
  ...
  * --discover reads dates from any control that looks like a
    bill or statement, wherever it sits on the page.
...
SAFETY (this is a utility account with a payment method on file):
  This module is strictly READ-ONLY. It opens the billing history, reads
  the list, and saves the PDFs SMUD already generated. It must NEVER
  activate any control that pays, sets up autopay, enrolls in a program
  or a rate plan, starts, stops or moves service, requests an extension
  or an arrangement, or edits any setting.
...
# ---------------------------------------------------------------------------
# HARD SAFETY GUARD. Tuned for electric utility, on top of the bank words. Never move
...
FORBIDDEN_CONTROL_RE = re.compile(
    ...
    r"start\s+service|stop\s+service|move\b|moving|budget\s+billing|level\s+pay|\bprograms?\b|rebates?\b|rate\s+(plan|option)|\bplans?\b|outage|report\b|solar|\bev\b|charging|schedule\b|arrangement|extension|donate|give\b)", re.I)

SAFE_DOC_CONTROL_RE = re.compile(
    r"(download|view|open|print|\bpdf\b|statement|document|\bletter\b|notice|"
    r"1099|1098|5498|tax\s+(form|document)|history|"
    r"\bbills?\b|billing\b|see\s+(more|all|older)|show\s+(more|all|older)|load\s+more)", re.I)
...
def _looks_like_billing(page) -> bool:
    ...
    try:
        ...
        return False
    return bool(re.search(r"bill(ing)?\s+(history|period|date)|past\s+bills|previous\s+bills|statement\s+(period|date)",
                          body, re.I))
...
def _fetch_pdf(page, href: str) -> Optional[bytes]:
    ...
    only on smud.org. None unless the answer is a PDF."""
...
SURVEY_LINK_RE = re.compile(
    r"^\s*((see|view|show)\s+)?(bills?|billing|bill(ing)?\s+history|statements?|view\s+bills?|documents)\s*$", re.I)
...
def survey(page, dwell_ms: int = 4000, max_follow: int = 6) -> dict:
    ...
    guard's verdict on each, and every JSON or PDF response smud.org sends
