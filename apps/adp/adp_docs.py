...
adp_site.py, which carries every guess about adp.com and says which ones
...
Usage:
    python adp_docs.py --login       verify connection to your browser
    python adp_docs.py --discover    list available documents
    python adp_docs.py --pilot       download the 5 newest, then stop
    python adp_docs.py --all         download everything in scope
    python adp_docs.py --resume      continue an interrupted run
    python adp_docs.py --verify      re-validate every saved PDF
    python adp_docs.py --diagnose    dump page structure (no downloads)
    python adp_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the Statements & Documents page and saves the PDFs ADP Workforce Now
...
log = logging.getLogger("adp_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in ADP Workforce Now tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! ADP Workforce Now appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9265")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. ADP's sign-in often goes through the employer's own identity
        # provider and asks for a code, so the browser already on the machine,
        # with its passkeys and password manager, is the right one.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in ADP Workforce Now browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but ADP Workforce Now shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a ADP Workforce Now page.
        Returns 1 if new."""
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available ADP Workforce Now documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='ADP Workforce Now', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        print("then attach it to the ADP Workforce Now issue on GitHub. No screenshot was taken.")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "ADP Workforce Now Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised ADP Workforce Now document downloader (read-only)")
