"""What makes the ADP Workforce Now app different, everything else is paperpull_core.
...
declaration of ADP Workforce Now's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair ADP Workforce Now's *page* behavior, edit `adp_site.py` instead.
"""
...
# One row per downloaded ADP Workforce Now document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    kind=DOCUMENT,
    folders=[
        # At ADP a "statement" IS a pay statement, so the shared classifier's
        # Statement category files here under the payroll wording.
