...
log = logging.getLogger("target_receipts.pdf")
...
# the receipts column on pages without stable ids/landmarks (Target's
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://www.target.com/">',
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Target, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Target/order details",
