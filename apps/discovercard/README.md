...
- **Nothing is clicked**, which also keeps the app clear of the neighboring
...
- **Delete-safe & multi-account** like every PaperPull app (`--config`,
  `paperpull discovercard add-account NAME`). Deleting a downloaded PDF does not make the next run fetch
