"""E-ZPass Virginia statement downloader (local, supervised).

Reads the View Online Statements list on the signed-in E-ZPass Virginia
portal, the same page fragment the link loads, asked for from inside the
page, and saves every monthly and quarterly statement PDF it lists.
Nothing on the page is clicked. The site layer, ezpassva_site.py, holds
every fact about ezpassva.com.

Usage:
    python ezpassva_docs.py --login       verify connection to your browser
    python ezpassva_docs.py --discover    list available documents
    python ezpassva_docs.py --pilot       download the 5 newest, then stop
    python ezpassva_docs.py --all         download everything in scope
    python ezpassva_docs.py --resume      continue an interrupted run
    python ezpassva_docs.py --verify      re-validate every saved PDF
    python ezpassva_docs.py --diagnose    dump page structure (no downloads)
    python ezpassva_docs.py --dry-run     plan filenames, save nothing
...
E-ZPass Virginia already generated. It never pays, replenishes, changes a
vehicle, transponder or card, subscribes to paper statements, or changes
any account setting. Everything stays on this machine; nothing is sent to any
external service.
"""
...
log = logging.getLogger("ezpassva_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. Category, date, title and account. E-ZPass has
        no document id, a statement is its kind (monthly or quarterly, in the
        title) plus its period. The account part is empty for this site and
        kept so the key reads like every other statement app's."""
        if self.document_id:
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in E-ZPass Virginia tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! E-ZPass Virginia appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9274")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first, the browser the portal was mapped
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in E-ZPass Virginia browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but E-ZPass Virginia shows a signed-out page.")
            print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
        elif ok:
            print("Success: connected and signed in to the E-ZPass portal.")
            print("Keep that browser window OPEN, then run:  paperpull ezpassva pilot")
        else:
            print("Connected, but I could not open the E-ZPass portal home page.")
            print("Open myaccount.ezpassva.com in that browser yourself, then run --diagnose.")
        self.close()
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        ...
        # E-ZPass lists one account per sign-in, so this is empty.
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Be on the portal, then ask for the statements list.
        ...
        self.discovery.save()
        log.info("Online Statements: %d statements, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            print("\n  !! E-ZPass Virginia answered with a sign-in page instead of a statement.")
            print("     Your session has expired. Sign in again in the open")
            print("     browser, then run:  paperpull ezpassva resume")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available E-ZPass Virginia documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='E-ZPass Virginia', text=text, extra=extra)
        if not path:
            return
    ...
    def write_survey(self) -> None:
        ...
        failure.write_survey(
            ...
            journal=self._journal,
            requests=self._requests,
            provider='E-ZPass Virginia')

    def cmd_diagnose(self):
        """Survey the portal page and what the statements list holds, and
        ...
        info = {"timestamp": now_iso(),
                "note": "What the E-ZPass Virginia statements list looks like, "
        ...
        print("then attach it to the E-ZPass Virginia issue on GitHub. No screenshot was taken.")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "E-ZPass Virginia Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised E-ZPass Virginia document downloader (read-only)")
    ...
    ap.add_argument("--type", help="Statement (the only kind E-ZPass offers)")
