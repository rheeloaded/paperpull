"""What makes the E-ZPass Virginia app different, everything else is paperpull_core.
...
declaration of E-ZPass Virginia's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair E-ZPass Virginia's *page* behavior, edit `ezpassva_site.py` instead.
"""
...
# One row per downloaded E-ZPass Virginia document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # A toll account issues no tax forms, so the folder is made only if
        # a title ever routes there, never left sitting empty.
    ...
    base_url="https://myaccount.ezpassva.com/",
