...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-01-02", "thing", "Statement") ==         "2026-01-02 Walmart Thing Statement.pdf"


def test_csv_filenames_are_unchanged(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.order_history_csv.name == "Walmart Order History.csv"
    assert paths.receipt_index_csv.name == "Walmart Receipt Index.csv"
...
def test_precreated_folders_are_unchanged(tmp_path):
    ...
    assert made == ['Backups', 'Diagnostics', 'In-Store', 'Invoices', 'Logs', 'Manual Review', 'Online']
