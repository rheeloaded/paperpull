...
log = logging.getLogger("walmart_receipts.pdf")
...
(() => {
  try {
    ...
    window.__targetReceiptsOriginalPrint = orig;
    window.__targetReceiptsPrintCalled = false;
    window.print = function () {
      window.__targetReceiptsPrintCalled = true;
      ...
      try {
        ...
        window.__targetReceiptsPrintHTML = html;
        try {
          window.top.__targetReceiptsPrintHTML = html;
          window.top.__targetReceiptsPrintCalled = true;
        } catch (e) {}
      } catch (e) {}
    };
  } catch (e) {}
...
(() => {
  try {
    if (window.__targetReceiptsOriginalPrint) {
      window.print = window.__targetReceiptsOriginalPrint;
    }
  } catch (e) {}
})();
"""
...
# the receipts column on pages without stable ids/landmarks (Walmart's
...
(needles) => {
  ...
  best.setAttribute('data-target-receipts-isolate', '1');
  return true;
}
"""
...
() => {
  ...
  let hidden = 0;
  const root = document.querySelector('[data-target-receipts-isolate]')
...
def isolate_online_receipts(page) -> None:
    ...
    try:
        ...
        pass
    if found:
        isolate_for_print(page, "[data-target-receipts-isolate='1']")
    else:
...
def was_print_called(page) -> bool:
    try:
        return bool(page.evaluate("() => window.__targetReceiptsPrintCalled === true"))
    except Exception:
        return False
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://www.walmart.com/">',
...
def get_print_snapshot(page) -> Optional[str]:
    ...
    try:
        html = page.evaluate("() => window.__targetReceiptsPrintHTML || null")
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Walmart, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Walmart/order details",
