"""UPS Billing Center invoice downloader (local, supervised).

Opens My Invoices in the signed-in UPS Billing Center, keeps the list the
page receives and the headers it sent, and saves each shipping invoice PDF
by asking for it the way the page does. Nothing on the page is clicked. The
site layer, ups_site.py, holds every fact about billing.ups.com.

Usage:
    python ups_docs.py --login       verify connection to your browser
    python ups_docs.py --discover    list available documents
    python ups_docs.py --pilot       download the 5 newest, then stop
    python ups_docs.py --all         download everything in scope
    python ups_docs.py --resume      continue an interrupted run
    python ups_docs.py --verify      re-validate every saved PDF
    python ups_docs.py --diagnose    dump page structure (no downloads)
    python ups_docs.py --dry-run     plan filenames, save nothing
...
PDFs UPS already generated. It never pays out, refunds, charges,
...
log = logging.getLogger("ups_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. UPS gives every invoice its own
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in UPS tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! UPS appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9277")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. ups.com runs bot protection and is
        # happiest in a real browser.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in UPS browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but UPS shows a signed-out page.")
            print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
        elif ok:
            print("Success: connected and signed in to the UPS Billing Center.")
            print("Keep that browser window OPEN, then run:  paperpull ups pilot")
        else:
            print("Connected, but I could not open the UPS Billing Center.")
            print("Open billing.ups.com in that browser yourself, then run --diagnose.")
        self.close()
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        ...
        # A UPS sign-in reads one account at a time, so this is empty.
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Be in the Billing Center, then open My Invoices and keep its answer.
        ...
        self.discovery.save()
        log.info("My Invoices: %d invoices, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            # UPS wants something only the account holder may give.
            # Never answered here.
            print(f"\n  !! {e}. Download it yourself in the Billing Center,")
            print("     My Invoices.")
            ...
            print("\n  !! UPS answered with a sign-in page instead of a document.")
            print("     Your session has expired. Sign in again in the open")
            print("     browser, then run:  paperpull ups resume")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available UPS documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='UPS', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(),
                "note": "What the UPS My Invoices list looks like, "
        ...
        print("then attach it to the UPS issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised UPS document downloader (read-only)")
