"""What makes the UPS app different, everything else is paperpull_core.
...
declaration of UPS's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair UPS's *page* behavior, edit `ups_site.py` instead.
"""
...
# One row per downloaded UPS document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    kind=DOCUMENT,
    folders=[
        # A shipping invoice classifies as a statement, it is what UPS bills.
    ...
    base_url="https://billing.ups.com/",
