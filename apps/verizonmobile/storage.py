"""What makes the Verizon Mobile app different, everything else is paperpull_core.
...
declaration of Verizon Mobile's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair Verizon Mobile's *page* behavior, edit `verizonmobile_site.py` instead.
"""
...
# One row per downloaded Verizon Mobile document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Verizon Mobile does not issue them, so it is created only if one ever
