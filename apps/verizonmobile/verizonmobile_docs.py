"""Verizon Mobile bill downloader (local, supervised). UNVERIFIED, see verizonmobile_site.py.

This app was written without a Verizon Mobile account so that someone who holds one
can test it without writing code. The orchestrator below is the same one
every other bill app runs on. What has not been proven is the site layer,
verizonmobile_site.py, which carries every guess about verizon.com and says which ones
...
Usage:
    python verizonmobile_docs.py --login       verify connection to your browser
    python verizonmobile_docs.py --discover    list available documents
    python verizonmobile_docs.py --pilot       download the 5 newest, then stop
    python verizonmobile_docs.py --all         download everything in scope
    python verizonmobile_docs.py --resume      continue an interrupted run
    python verizonmobile_docs.py --verify      re-validate every saved PDF
    python verizonmobile_docs.py --diagnose    dump page structure (no downloads)
    python verizonmobile_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the bill history and saves the PDFs Verizon Mobile
already generated. It never pays a bill, changes a plan or a line, or
changes any account setting. Everything stays on this machine; nothing is
...
log = logging.getLogger("verizonmobile_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. Category, date and title. Verizon Mobile's bill list is
        read from the page, so there is no document id to store, and a
        bill's date is what tells one from the next."""
        if self.document_id:
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Verizon Mobile tab.
        ...
        # Whatever verizon.com does on a bill click, a download event, a PDF
        # response or a new tab, is caught in verizonmobile_site.download_bill. No CDP
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Verizon Mobile appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9255")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. verizon.com runs bot protection that
        # walls the Playwright build of Chromium. The Fios app proved it.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Verizon Mobile browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Verizon Mobile shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Verizon Mobile page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Verizon Mobile lists past bills on a history page. Open it and read the bill
        # date from every control that fetches a bill.
    ...
    def download_one(self, page, doc: Document, filename: str):
        """Save one bill. The site layer fetches the row's PDF link from
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Verizon Mobile documents ({scope}).")
    ...
    def cmd_diagnose(self):
        """Survey the billing area and write a file a tester can attach to
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "Verizon Mobile app built without an account. This survey is what "
        ...
        print("then attach it to the Verizon Mobile issue on GitHub. No screenshot was taken.")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Verizon Mobile Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Verizon Mobile document downloader (read-only)")
