...
log = logging.getLogger("usaa_docs.pdf")
...
(() => {
  try {
    ...
    window.__usaaDocsOriginalPrint = orig;
    window.__usaaDocsPrintCalled = false;
    window.print = function () {
      window.__usaaDocsPrintCalled = true;
      ...
      try {
        ...
        window.__usaaDocsPrintHTML = html;
        try {
          window.top.__usaaDocsPrintHTML = html;
          window.top.__usaaDocsPrintCalled = true;
        } catch (e) {}
      } catch (e) {}
    };
  } catch (e) {}
...
(() => {
  try {
    if (window.__usaaDocsOriginalPrint) {
      window.print = window.__usaaDocsOriginalPrint;
    }
  } catch (e) {}
})();
"""
...
# the receipts column on pages without stable ids/landmarks (USAA's
...
(needles) => {
  ...
  best.setAttribute('data-usaa-docs-isolate', '1');
  return true;
}
"""
...
() => {
  ...
  let hidden = 0;
  const root = document.querySelector('[data-usaa-docs-isolate]')
...
def isolate_online_receipts(page) -> None:
    ...
    try:
        ...
        pass
    if found:
        isolate_for_print(page, "[data-usaa-docs-isolate='1']")
    else:
...
def was_print_called(page) -> bool:
    try:
        return bool(page.evaluate("() => window.__usaaDocsPrintCalled === true"))
    except Exception:
        return False
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://www.usaa.com/">',
...
def get_print_snapshot(page) -> Optional[str]:
    ...
    try:
        html = page.evaluate("() => window.__usaaDocsPrintHTML || null")
...
def clear_print_snapshot(page) -> None:
    try:
        page.evaluate("() => { window.__usaaDocsPrintHTML = null; "
                      "window.__usaaDocsPrintCalled = false; }")
    except Exception:
        pass
...
def is_zip(path: Path) -> bool:
    """Some USAA tax forms (e.g. 1099-R) download as a ZIP holding the
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (USAA, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention USAA/order details",
