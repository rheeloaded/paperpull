"""Filesystem, filename, CSV, and progress storage for the USAA receipt downloader.
...
DEFAULT_OUTPUT_DIR = Path(r"C:\Users\YOU\Downloads\USAA Statements")
...
# One row per downloaded USAA document. Deliberately records NO
...
def load_config(path: Optional[Path] = None) -> dict:
    ...
    cfg.setdefault("profile_dir", str(PROJECT_DIR / "usaa-browser-profile"))
...
class Paths:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.document_index_csv = self.root / "USAA Document Index.csv"
        self.order_history_csv = self.root / "USAA Order History.csv"
        self.receipt_index_csv = self.root / "USAA Receipt Index.csv"
    ...
    def all_dirs(self) -> List[Path]:
        ...
        return [self.root, self.statements, self.tax_documents,
                self.insurance_documents, self.manual_review, self.logs,
...
def build_pdf_filename(purchase_date: str, summary: str,
    ...
    """YYYY-MM-DD USAA <Summary> <Receipt|Invoice>[ (i of n)].pdf"""
    ...
    base = f"{date} {who}USAA {summary} {document_type}"
