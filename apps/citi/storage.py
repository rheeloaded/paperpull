"""What makes the Citi app different, everything else is paperpull_core.
...
declaration of Citi's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair Citi's *page* behavior, edit `citi_site.py` instead.
"""
...
# One row per downloaded Citi document. Deliberately records NO account
