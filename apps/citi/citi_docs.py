"""Citi credit card statement downloader (local, supervised).

Reads the Account Statements list for every card on the signed-in Citi
Online session through the same JSON API the page uses, called from inside
the page, and saves each month's statement PDF. Nothing on the page is
clicked. The site layer, citi_site.py, holds every fact about citi.com.

Usage:
    python citi_docs.py --login       verify connection to your browser
    python citi_docs.py --discover    list available documents
    python citi_docs.py --pilot       download the 5 newest, then stop
    python citi_docs.py --all         download everything in scope
    python citi_docs.py --resume      continue an interrupted run
    python citi_docs.py --verify      re-validate every saved PDF
    python citi_docs.py --diagnose    dump page structure (no downloads)
    python citi_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement

READ-ONLY: this tool only reads the Account Statements list and saves the
PDFs Citi already generated. It never pays, transfers, requests a credit
line change, adds a user, redeems anything, or changes any account
...
log = logging.getLogger("citi_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.document_id = document_id  # unused, card plus date is the identity
    ...
    @property
    def key(self) -> str:
        """Stable identity. Category, date, title and card. Citi has no
        document id, a statement is the card plus its closing date."""
        if self.document_id:
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Citi tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Citi appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9256")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. Citi Online runs bot protection that
        # is happiest in a real browser, and one signed-in tab is all the
        # API calls need.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Citi browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Citi shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one statement (a RawDoc) the API listed. Returns 1 if
        new. The card's label and its opaque account id ride along, the
        download needs the id and the filename carries the label."""
        ...
        if floor and (not date or date < floor):
            self.stats["skipped_out_of_scope"] += 1
            return 0
        # The card's name goes into the summary, so two cards' statements
        # for the same month get two filenames instead of a (2).
        ...
        if self.discovery.get(doc.key) is None:
            ...
            return 1
        # refresh the account id the download needs, in case it changed
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Open Account Statements, then ask the API for every card and
        # every statement it lists for each.
        ...
        self.discovery.save()
        log.info("Account Statements: %d statements, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        """Save one statement. The site layer asks the API for the PDF from
        ...
        try:
            saved = site.download_bill(page, self._dl_dir, doc.date, out_path,
                                       account_id=doc.href, account=doc.account)
        except site.SessionExpired:
            ...
            print("\n  !! Citi answered with a sign-in page instead of a statement.")
            print("     Your session has expired. Sign in again in the open")
            print("     browser, then run:  paperpull citi resume")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Citi documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Citi', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(),
                "note": "What the Citi statements page and its API look like, "
        ...
        print("then attach it to the Citi issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Citi document downloader (read-only)")
    ...
    ap.add_argument("--type", help="Statement (the only kind a card offers)")
