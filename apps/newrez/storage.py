"""What makes the Newrez app different, everything else is paperpull_core.
...
declaration of Newrez's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair Newrez's *page* behavior, edit `newrez_site.py` instead.
"""
...
# One row per downloaded Newrez document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Newrez does not issue them, so it is created only if one ever
    ...
    base_url="https://myaccount.newrez.com/",
