...
Usage:
    ...
    python newrez_docs.py --discover    list available documents
    python newrez_docs.py --pilot       download the 5 newest, then stop
    python newrez_docs.py --all         download everything in scope
    python newrez_docs.py --resume      continue an interrupted run
    python newrez_docs.py --verify      re-validate every saved PDF
    python newrez_docs.py --diagnose    dump page structure (no downloads)
    python newrez_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the documents page and saves the PDFs Newrez
...
log = logging.getLogger("newrez_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Newrez tab.
        ...
        self._dl_dir = Path(self.config["output_dir"]) / ".newrez-downloads"
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Newrez appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9257")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first, since Newrez's portal is happiest in a real browser.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Newrez browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Newrez shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Newrez page.
        Returns 1 if new."""
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Newrez documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Newrez', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "Newrez app built without an account. This survey is what "
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Newrez document downloader (read-only)")
