"""What makes the E*TRADE app different, everything else is paperpull_core.
...
declaration of E*TRADE's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair E*TRADE's *page* behavior, edit `etrade_site.py` instead.
"""
...
# One row per downloaded E*TRADE document. Deliberately records NO account
...
SPEC = AppSpec(
    provider="ETRADE",  # E*TRADE, but a star is not a filename character
    project_dir=Path(__file__).resolve().parent,
    kind=DOCUMENT,
    folders=[
        Folder("statements", "Statements"),
        # A yearly tax form is expected, so the folder is made up front.
        Folder("tax_documents", "Tax Documents"),
        # Reachable if "Insurance Document" is added to document_types, but
        # E*TRADE does not issue them, so it is created only if one ever
