...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-01-02", "thing", "Statement") ==         "2026-01-02 ETRADE Thing Statement.pdf"


def test_csv_filename_is_unchanged(tmp_path):
    assert storage.Paths(tmp_path).document_index_csv.name == "ETRADE Document Index.csv"
