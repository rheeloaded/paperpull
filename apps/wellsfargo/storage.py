"""What makes the Wells Fargo app different, everything else is paperpull_core.
...
declaration of Wells Fargo's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair Wells Fargo's *page* behavior, edit `wellsfargo_site.py` instead.
"""
...
# One row per downloaded Wells Fargo document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # A bank issues 1099-INT and 1098 every year, so the folder is made
        # up front.
        Folder("tax_documents", "Tax Documents"),
        # Reachable if "Insurance Document" is added to document_types, but
        # Wells Fargo does not issue them, so it is created only if one ever
