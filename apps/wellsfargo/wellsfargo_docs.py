"""Wells Fargo statement and tax document downloader (local, supervised). UNVERIFIED, see wellsfargo_site.py.

This app was written without a Wells Fargo account so that someone who holds one
...
wellsfargo_site.py, which carries every guess about wellsfargo.com and says which ones
...
Usage:
    python wellsfargo_docs.py --login       verify connection to your browser
    python wellsfargo_docs.py --discover    list available documents
    python wellsfargo_docs.py --pilot       download the 5 newest, then stop
    python wellsfargo_docs.py --all         download everything in scope
    python wellsfargo_docs.py --resume      continue an interrupted run
    python wellsfargo_docs.py --verify      re-validate every saved PDF
    python wellsfargo_docs.py --diagnose    dump page structure (no downloads)
    python wellsfargo_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the Statements & Documents page and saves the PDFs Wells Fargo
...
log = logging.getLogger("wellsfargo_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in Wells Fargo tab.
        ...
        self._dl_dir = Path(self.config["output_dir"]) / ".wellsfargo-downloads"
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Wells Fargo appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9253")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. wellsfargo.com runs bot protection
        # that walls the Playwright build of Chromium the way Verizon's does.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Wells Fargo browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Wells Fargo shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from a Wells Fargo page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Open the Statements & Documents page and read the date from every control that
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        # Open the Statements & Documents page and let the site layer find and fetch this one.
        if not site.goto_documents(page):
            self.check_session(page)
            site.goto_documents(page)
        trace: list = []
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Wells Fargo documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Wells Fargo', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        info = {"timestamp": now_iso(), "unverified": True,
                "note": "Wells Fargo app built without an account. This survey is what "
        ...
        print("then attach it to the Wells Fargo issue on GitHub. No screenshot was taken.")
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Wells Fargo Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Wells Fargo document downloader (read-only)")
