"""Wells Fargo classification, the guard, and the survey's promises.
...
def test_titles_classify_the_way_the_filenames_need():
    for title, summary, cat in [('Monthly Statement - August 31, 2026', 'Monthly Statement', 'STATEMENT'), ('Checking statement', 'Account Statement', 'STATEMENT'), ('Account Statement - July 2026', 'Account Statement', 'STATEMENT'), ('1099-INT', '1099-INT Tax Form', 'TAX'), ('Form 1098', '1098 Tax Form', 'TAX'), ('Tax Document - 2025', 'Tax Document', 'TAX')]:
...
def test_the_noise_is_skipped():
    for t in ['Privacy Notice', 'Fee schedule', 'Account agreement', 'Check image']:
...
def test_the_survey_follows_only_documents_links():
    for text in ['Statements & Documents', 'Statements and Documents', 'Statements', 'Tax Documents', 'View statements']:
...
def test_only_the_providers_own_hosts():
    for u in ['https://connect.secure.wellsfargo.com/edocs/statements', 'https://www.wellsfargo.com/x', 'https://oam.wellsfargo.com/x']:
        assert site.is_safe_url(u), u
    for u in ['https://wellsfargo.com.evil.test/s.pdf', 'http://connect.secure.wellsfargo.com/s.pdf', 'https://user@wellsfargo.com/s.pdf']:
