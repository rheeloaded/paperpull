"""What makes the Golden 1 app different, everything else is paperpull_core.
...
declaration of Golden 1's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair Golden 1's *page* behavior, edit `golden1_site.py` instead.
"""
...
# One row per downloaded Golden 1 document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Golden 1 does not issue them, so it is created only if one ever
    ...
    base_url="https://login.golden1.com/",
