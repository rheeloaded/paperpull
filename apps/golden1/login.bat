@echo off
cd /d "%~dp0"
rem  login.bat          -> your account (config.json, port 9258)
rem  login.bat spouse   -> config.spouse.json (own folders, profile, port)
if "%~1"=="" (set "CFG=") else (set "CFG=--config config.%~1.json")
echo ============================================================
echo  Golden 1 Documents - sign in
echo ============================================================
if not "%~1"=="" echo Account: %~1
echo.
echo Your own Edge or Chrome will open, with a separate profile. Then:
echo   1. Sign in to Golden 1 (do all the 2FA / device approval yourself).
...
echo   4. Then run:  paperpull golden1 diagnose   (a survey for the maintainer)
echo.
...
echo setting.
echo.
.venv\Scripts\python.exe golden1_docs.py --open-browser %CFG%
echo.
pause
