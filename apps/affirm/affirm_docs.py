"""Affirm statement downloader (local, supervised).

Usage:
    python affirm_docs.py --login       verify connection to your browser
    python affirm_docs.py --discover    list available documents
    python affirm_docs.py --pilot       download the 5 newest, then stop
    python affirm_docs.py --all         download everything in scope
    python affirm_docs.py --resume      continue an interrupted run
    python affirm_docs.py --verify      re-validate every saved PDF
    python affirm_docs.py --diagnose    dump page structure (no downloads)
    python affirm_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the statements and documents pages and downloads
PDFs that Affirm already generated. It never moves money between funds, changes
...
Mapped 2026-09-18. See the STATUS block in affirm_site.py for the API, the
...
log = logging.getLogger("affirm_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Kind, account and date, never a site id. The id is looked up fresh
        at download time. myPay's ids died with the session, and there is no
        reason to find out the hard way whether Affirm's do."""
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's signed-in Affirm tab; a fresh one is unauthenticated.
            # Matched by parsed host, not by substring: "affirm.com" in the URL
            # would also accept nottsp.com.example, and would have picked an
            # unrelated tab as the work page when no Affirm tab was open.
            live = [p for p in ctx.pages if not p.is_closed()]
            mine = [p for p in live if site.is_safe_url(p.url or "")]
            if mine:
                self._work_page = mine[0]
            else:
                log.warning("No Affirm tab is open. Sign in with login.bat and "
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Affirm appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Affirm browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Affirm shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has Affirm documents from 2023 and earlier).
    ...
    def _record_statement_doc(self, d: dict, occurrence: int = 0) -> int:
        """Record one site row {title, date, account, category, item_id, client_id}. Returns 1 if new."""
        ...
        if d.get("category"):
            # The site says which kind a document is. Rules only choose the
            # wording inside the filename.
            category = d["category"]
            if category == storage.AGREEMENT and summary in ("", "Other Document", "Monthly Statement"):
                summary = "Loan Agreement"
            elif category == storage.STATEMENT and summary in ("", "Other Document"):
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            print("\n  !! Affirm answered with a sign-in error instead of a document.")
    ...
    def _write_row(self, doc: Document, status: str, processing: str):
        ...
        self.index_csv.append_rows([{
            ...
            # No URL exists for a Affirm document: it is fetched by a
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Affirm documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Affirm', text=text, extra=extra)
        if not path:
            return
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Affirm document downloader (read-only)")
