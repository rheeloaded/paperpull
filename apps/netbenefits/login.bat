@echo off
cd /d "%~dp0"
rem  login.bat          -> your account (config.json, port 9249)
rem  login.bat spouse   -> config.spouse.json (own folders, profile, port)
if "%~1"=="" (set "CFG=") else (set "CFG=--config config.%~1.json")
echo ============================================================
echo  NetBenefits Documents - sign in
echo ============================================================
if not "%~1"=="" echo Account: %~1
echo.
echo A browser window will open on fidelity.com. Then:
echo   1. Sign in to NetBenefits yourself, including the two-factor prompt.
...
echo   4. Then run:  paperpull netbenefits diagnose   (a safe look, downloads nothing)
echo.
...
echo.
.venv\Scripts\python.exe netbenefits_docs.py --open-browser %CFG%
echo.
pause
