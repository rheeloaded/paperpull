"""Fidelity NetBenefits statement downloader (local, supervised).

Usage:
    python netbenefits_docs.py --login       verify connection to your browser
    python netbenefits_docs.py --discover    list available documents
    python netbenefits_docs.py --pilot       download the 5 newest, then stop
    python netbenefits_docs.py --all         download everything in scope
    python netbenefits_docs.py --resume      continue an interrupted run
    python netbenefits_docs.py --verify      re-validate every saved PDF
    python netbenefits_docs.py --diagnose    dump page structure (no downloads)
    python netbenefits_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only makes statements on the Statements page and renders
them to PDF. It never moves money between funds, changes
...
Mapped 2026-09-18. See the STATUS block in netbenefits_site.py for the API, the
...
log = logging.getLogger("netbenefits_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's signed-in NetBenefits tab; a fresh one is unauthenticated.
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            print("\n  !! NetBenefits answered with a sign-in error instead of a document.")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='NetBenefits', text=text, extra=extra)
        if not path:
            return
