...
log = logging.getLogger("navyfederal_docs.pdf")
...
(() => {
  try {
    ...
    window.__navyfederalDocsOriginalPrint = orig;
    window.__navyfederalDocsPrintCalled = false;
    window.print = function () {
      window.__navyfederalDocsPrintCalled = true;
      ...
      try {
        ...
        window.__navyfederalDocsPrintHTML = html;
        try {
          window.top.__navyfederalDocsPrintHTML = html;
          window.top.__navyfederalDocsPrintCalled = true;
        } catch (e) {}
      } catch (e) {}
    };
  } catch (e) {}
...
(() => {
  try {
    if (window.__navyfederalDocsOriginalPrint) {
      window.print = window.__navyfederalDocsOriginalPrint;
    }
  } catch (e) {}
})();
"""
...
# the receipts column on pages without stable ids/landmarks (Navy Federal's
...
(needles) => {
  ...
  best.setAttribute('data-navyfederal-docs-isolate', '1');
  return true;
}
"""
...
() => {
  ...
  let hidden = 0;
  const root = document.querySelector('[data-navyfederal-docs-isolate]')
...
def isolate_online_receipts(page) -> None:
    ...
    try:
        ...
        pass
    if found:
        isolate_for_print(page, "[data-navyfederal-docs-isolate='1']")
    else:
...
def was_print_called(page) -> bool:
    try:
        return bool(page.evaluate("() => window.__navyfederalDocsPrintCalled === true"))
    except Exception:
        return False
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://www.navyfederal.com/">',
...
def get_print_snapshot(page) -> Optional[str]:
    ...
    try:
        html = page.evaluate("() => window.__navyfederalDocsPrintHTML || null")
...
def clear_print_snapshot(page) -> None:
    try:
        page.evaluate("() => { window.__navyfederalDocsPrintHTML = null; "
                      "window.__navyfederalDocsPrintCalled = false; }")
    except Exception:
        pass
...
def is_zip(path: Path) -> bool:
    """Some Navy Federal tax forms (e.g. 1099-R) download as a ZIP holding the
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Navy Federal, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Navy Federal/order details",
