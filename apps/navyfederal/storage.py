"""Filesystem, filename, CSV, and progress storage for the Navy Federal receipt downloader.
...
DEFAULT_OUTPUT_DIR = Path(r"C:\Users\YOU\Downloads\Navy Federal Statements")
...
# One row per downloaded Navy Federal document. Deliberately records NO
...
def load_config(path: Optional[Path] = None) -> dict:
    ...
    cfg.setdefault("profile_dir", str(PROJECT_DIR / "navyfederal-browser-profile"))
...
class Paths:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.document_index_csv = self.root / "Navy Federal Document Index.csv"
        self.order_history_csv = self.root / "Navy Federal Order History.csv"
        self.receipt_index_csv = self.root / "Navy Federal Receipt Index.csv"
    ...
    def all_dirs(self) -> List[Path]:
        return [self.root, self.statements, self.tax_documents,
                self.insurance_documents, self.other_documents,
...
def build_pdf_filename(purchase_date: str, summary: str,
    ...
    """YYYY-MM-DD Navy Federal <Summary> <Receipt|Invoice>[ (i of n)].pdf"""
    ...
    base = f"{date} {who}Navy Federal {summary} {document_type}"
