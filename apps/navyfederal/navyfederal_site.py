"""ALL Navy Federal.com selectors, URLs, and page behavior live here.

When Navy Federal changes its site, repair this file only.

SAFETY (this is a BANK):
  This module is strictly READ-ONLY. It navigates to the document/statement
  areas, reads a list of documents, and downloads the PDFs Navy Federal already
...
# Navy Federal "My Documents" is a table: Document title | Date delivered | Account |
...
def collect_documents(page) -> List[RawDoc]:
    """Collect Navy Federal document rows currently rendered in the table."""
    docs: List[RawDoc] = []
    seen = set()
    try:
...
def collect_group_rows(page):
    try:
        return page.main_frame.evaluate(_NFCU_ROW_JS)
    except Exception:
        return []
