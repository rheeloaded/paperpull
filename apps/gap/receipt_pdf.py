...
log = logging.getLogger("gap_receipts.pdf")
...
(() => {
  try {
    ...
    window.__gapReceiptsOriginalPrint = orig;
    window.__gapReceiptsPrintCalled = false;
    window.print = function () {
      window.__gapReceiptsPrintCalled = true;
      ...
      try {
        ...
        window.__gapReceiptsPrintHTML = html;
        try {
          window.top.__gapReceiptsPrintHTML = html;
          window.top.__gapReceiptsPrintCalled = true;
        } catch (e) {}
      } catch (e) {}
    };
  } catch (e) {}
...
(() => {
  try {
    if (window.__gapReceiptsOriginalPrint) {
      window.print = window.__gapReceiptsOriginalPrint;
    }
  } catch (e) {}
})();
"""
...
# the receipts column on pages without stable ids/landmarks (Gap's
...
(needles) => {
  ...
  best.setAttribute('data-gap-receipts-isolate', '1');
  return true;
}
"""
...
() => {
  ...
  let hidden = 0;
  const root = document.querySelector('[data-gap-receipts-isolate]')
...
def isolate_online_receipts(page) -> None:
    ...
    try:
        ...
        pass
    if found:
        isolate_for_print(page, "[data-gap-receipts-isolate='1']")
    else:
...
def was_print_called(page) -> bool:
    try:
        return bool(page.evaluate("() => window.__gapReceiptsPrintCalled === true"))
    except Exception:
        return False
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://secure-www.gap.com/">',
...
def get_print_snapshot(page) -> Optional[str]:
    ...
    try:
        html = page.evaluate("() => window.__gapReceiptsPrintHTML || null")
...
def clear_print_snapshot(page) -> None:
    try:
        page.evaluate("() => { window.__gapReceiptsPrintHTML = null; "
                      "window.__gapReceiptsPrintCalled = false; }")
    except Exception:
        pass
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Gap, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Gap/order details",
