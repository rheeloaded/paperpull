...
att_site.py, which carries every guess about att.com and says which ones
...
Usage:
    python att_docs.py --login       verify connection to your browser
    python att_docs.py --discover    list available documents
    python att_docs.py --pilot       download the 5 newest, then stop
    python att_docs.py --all         download everything in scope
    python att_docs.py --resume      continue an interrupted run
    python att_docs.py --verify      re-validate every saved PDF
    python att_docs.py --diagnose    dump page structure (no downloads)
    python att_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the bill history and saves the PDFs AT&T
...
log = logging.getLogger("att_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. Category, date and title. AT&T's bill list is
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in AT&T tab.
        ...
        # Whatever att.com does on a bill click, a download event, a PDF
        # response or a new tab, is caught in att_site.download_bill. No CDP
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! AT&T appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9252")
        profile = self.config["profile_dir"]
        url = site.URLS.get("login") or site.URLS.get("documents") or site.URLS["home"]
        # A real Edge or Chrome first. att.com runs Akamai bot protection,
        # which walls the Playwright build of Chromium the way Verizon's does.
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in AT&T browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but AT&T shows a signed-out page.")
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        """Record one scraped document (a RawDoc) from an AT&T page.
        Returns 1 if new."""
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # AT&T lists past bills on a history page. Open it and read the bill
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available AT&T documents ({scope}).")
    ...
    def cmd_diagnose(self):
        ...
        print("then attach it to the AT&T issue on GitHub. No screenshot was taken.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised AT&T document downloader (read-only)")
