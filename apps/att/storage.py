"""What makes the AT&T app different, everything else is paperpull_core.
...
declaration of AT&T's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair AT&T's *page* behavior, edit `att_site.py` instead.
"""
...
# One row per downloaded AT&T document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # AT&T does not issue them, so it is created only if one ever
