"""Lowe's purchase-history & receipt downloader (local, supervised).

Usage:
    python lowes_receipts.py --login
    python lowes_receipts.py --discover
    python lowes_receipts.py --pilot            (newest few of each kind)
    python lowes_receipts.py --all
    python lowes_receipts.py --resume
    python lowes_receipts.py --verify
    python lowes_receipts.py --review-names
    python lowes_receipts.py --diagnose [--order-number N]
    python lowes_receipts.py --dry-run
...
log = logging.getLogger("lowes_receipts")
...
class App:
    ...
    def browser(self):
        ...
        never scripted. Lowe's sits behind bot protection, which is why the
        browser is the person's own Edge or Chrome, and no stealth or
        evasion is used anywhere. Set "cdp_url" to ""
        ...
        if cdp_url:
            ...
            # window Lowe's opens to print also has window.print() intercepted
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Lowe's appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9271")
    ...
    def cmd_login(self):
        if self.config.get("cdp_url"):
            ...
            print("Checking the connection to your signed-in Lowe's browser...\n")
            page = self.page()
            site.goto_orders(page)
            challenge = site.detect_security_challenge(page)
            if challenge:
                ...
                print("Connected, but Lowe's shows the signed-out page.")
                print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
            else:
                print("Success: connected to your signed-in Lowe's session.")
                print("Keep that browser window OPEN, then run the diagnostics or pilot.")
            self.close()
            return
        print("Opening lowes.com in a dedicated supervised browser profile.")
        ...
        if site.looks_signed_out(page):
            ...
            print("Sign in in the browser, then run:  python lowes_receipts.py --login")
        else:
            print("Signed-in session detected.")
        self.close()

    def cmd_discover(self, types: Optional[List[str]] = None, quiet: bool = False) -> dict:
        """Discovery pass: the whole purchase history, online orders and
        store purchases together, one page at a time by its own address.

        The range menu's "All" and a page number are both in the address,
        so nothing is pressed. A page is waited on until its cards are
        drawn, since one read too soon is empty and would end the walk
        early. The walk stops at the last page the pager shows, or at the
        first page that holds nothing new."""
        page = self.page()
        ...
        cards = []
        seen = set()
        showing = site.goto_orders(page, 1)
        self.check_session(page)
        last = site.page_count(page)
        for page_no in range(1, max(last, 1) + 1):
            if page_no > 1:
                showing = site.goto_orders(page, page_no)
                self.check_session(page)
            found = site.collect_cards(page)
            fresh = [c for c in found if c.number not in seen]
            log.info("History page %d of %d: %d showing, %d card(s), %d new",
                     page_no, last, showing, len(found), len(fresh))
            if not fresh:
                if page_no == 1 and site.history_state(page) == "empty":
                    print("\nLowe's says this account has no purchases.")
                break
            for c in fresh:
                seen.add(c.number)
            cards.extend(fresh)
            # With a floor set, the history is newest first, so a page that
            # ends before the floor is the last one worth reading.
            if floor:
                dates = [p.purchase_date for p in map(site.card_to_purchase, fresh)
                         if p and p.purchase_date]
        ...
        for card in cards:
            ...
            if self.discovery.get(key) is None:
                ...
                n_new[purchase.purchase_type] += 1
            else:
        ...
        if not quiet:
            ...
            if dropped_by_scope:
                print(f"  {dropped_by_scope} fell outside the scope you set.")
            by_year = {}
            for r in all_recs:
    ...
    def _save_receipt(self, page, purchase: Purchase) -> bool:
        """Save the Lowe's details page as a verified PDF receipt.

        The details page IS the receipt, the store or delivery address,
        every item with its item and model numbers, the payment method and
        the total billed. process_one has already opened it, and the page's
        own heading must name this purchase before anything is saved, so a
        page that answered for another purchase is never filed under this
        one. Nothing is clicked, Print Details included.
        """
        if site.looks_signed_out(page):
            print("  Lowe's is asking you to verify your sign-in.")
        ...
        if not site.receipt_is_present(page):
            ...
            self._write_csv_rows(purchase,
                                 ...
                                 notes_extra="Details page did not fill in")
            self.stats["no_receipt"] += 1
            self.stats["manual_review"] += 1
            self.write_failure("open the receipt", "the details page never showed Total Billed")
            print("  The details page did not fill in - marked for manual review.")
            return False

        shown = site.details_number(page)
        if shown != purchase.order_number:
            # Refused, not saved. A receipt filed under another purchase's
            # name is worse than none, because nobody looks for it.
            self._record_state(purchase, State.NEEDS_MANUAL_REVIEW,
                               notes="The details page named a different purchase")
            self.stats["wrong_document"] = self.stats.get("wrong_document", 0) + 1
            self.stats["manual_review"] += 1
            self.write_failure("check the receipt", "the details page named a different purchase")
            print("  The details page is for a different purchase - nothing was saved.")
            return False

        # A return keeps its own kind, so its file says what it is.
        purchase.document_type = "Return" if purchase.document_type == "Return" else "Receipt"
    ...
    def _capture_document(self, target_page, purchase: Purchase,
                          out_path: Path) -> None:
        """Render the Lowe's details receipt to PDF.

        Printed as it stands the page is four pages of navigation and
        promotions around the receipt. site.isolate_receipt hides
        everything but the receipt block first, a display change in the
        local page only, gone on the next navigation, which leaves a clean
        one-page receipt for CDP Page.printToPDF.
        """
    ...
    def cmd_pilot(self):
        ...
        selected: List[Purchase] = (
            self._select_purchases(IN_STORE, limit=self.config.get("pilot_instore", 3))
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider="Lowe's", text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        # The history first, every page of it, since a purchase missing
        # from the list is invisible from any one receipt.
        try:
            history = {}
            site.goto_orders(page, 1)
            last = site.page_count(page)
            for page_no in range(1, max(last, 1) + 1):
                if page_no > 1:
                    site.goto_orders(page, page_no)
                history["page_%d" % page_no] = site.history_survey(page)
            after = {"cards_collected": sum(h.get("cards_collected", 0) for h in history.values()),
                     "became_purchases": sum(len(h.get("purchases", [])) for h in history.values())}
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Local supervised Lowe's receipt downloader")
    modes = [
        ("login", "open browser for manual Lowe's sign-in"),
        ("discover", "discovery pass only; writes discovery.json"),
        ("pilot", "pilot: newest few of each kind"),
        ...
        ("diagnose", "inspect one purchase of each kind, write diagnostics"),
...
def main(argv=None):
    ...
    try:
        if getattr(args, "open_browser", False):
            ...
            app.cmd_run([IN_STORE, ONLINE], "all")
        elif args.resume:
            app.cmd_resume()
        elif args.verify:
            app.cmd_verify()
            ...
            app.cmd_run([IN_STORE, ONLINE], "dry-run")
        else:
