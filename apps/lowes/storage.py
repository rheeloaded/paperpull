"""What makes the Lowe's app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Lowe's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair Lowe's *page* behavior, edit `lowes_site.py` instead.
"""
...
SPEC = AppSpec(
    ...
    csv_files=[
        CsvSpec("order_history_csv", "Lowe's Order History.csv", ORDER_HISTORY_COLUMNS),
        CsvSpec("receipt_index_csv", "Lowe's Receipt Index.csv", RECEIPT_INDEX_COLUMNS),
    ],
    config_defaults={
        "pilot_online": 3,
        "pilot_instore": 3,
