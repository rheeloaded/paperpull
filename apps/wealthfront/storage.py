"""What makes the Wealthfront app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Wealthfront's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair Wealthfront's *page* behaviour, edit `wealthfront_site.py` instead.
"""
...
SPEC = AppSpec(
    ...
    csv_files=[
        CsvSpec("document_index_csv", "Wealthfront Document Index.csv", DOCUMENT_INDEX_COLUMNS),
    ],
    config_defaults={
    ...
    base_url="https://www.wealthfront.com/",
