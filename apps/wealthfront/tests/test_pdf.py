...
def test_zip_detection_and_extraction(tmp_path):
    """Wealthfront delivers some tax forms (e.g. 1099-R) as a ZIP holding the
    ...
    assert is_zip(zpath)
    out = tmp_path / "2025-12-31 Wealthfront 1099-R Tax Form.pdf"
    zpath.replace(out)
...
def test_image_based_pdf_not_rejected_for_no_text(tmp_path):
    ...
    p = tmp_path / "img.pdf"
    make_pdf(p)
    r = validate_pdf(p, min_bytes=1000, expect_tokens=["wealthfront", "12345"])
    assert r.ok, r.reason
