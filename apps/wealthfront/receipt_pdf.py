...
log = logging.getLogger("wealthfront_docs.pdf")
...
(() => {
  try {
    ...
    window.__wfDocsOriginalPrint = orig;
    window.__wfDocsPrintCalled = false;
    window.print = function () {
      window.__wfDocsPrintCalled = true;
      ...
      try {
        ...
        window.__wfDocsPrintHTML = html;
        try {
          window.top.__wfDocsPrintHTML = html;
          window.top.__wfDocsPrintCalled = true;
        } catch (e) {}
      } catch (e) {}
    };
  } catch (e) {}
...
(() => {
  try {
    if (window.__wfDocsOriginalPrint) {
      window.print = window.__wfDocsOriginalPrint;
    }
  } catch (e) {}
})();
"""
...
# the receipts column on pages without stable ids/landmarks (Wealthfront's
...
(needles) => {
  ...
  best.setAttribute('data-wf-docs-isolate', '1');
  return true;
}
"""
...
() => {
  ...
  let hidden = 0;
  const root = document.querySelector('[data-wf-docs-isolate]')
...
def isolate_online_receipts(page) -> None:
    ...
    try:
        ...
        pass
    if found:
        isolate_for_print(page, "[data-wf-docs-isolate='1']")
    else:
...
def was_print_called(page) -> bool:
    try:
        return bool(page.evaluate("() => window.__wfDocsPrintCalled === true"))
    except Exception:
        return False
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://www.wealthfront.com/">',
...
def get_print_snapshot(page) -> Optional[str]:
    ...
    try:
        html = page.evaluate("() => window.__wfDocsPrintHTML || null")
...
def clear_print_snapshot(page) -> None:
    try:
        page.evaluate("() => { window.__wfDocsPrintHTML = null; "
                      "window.__wfDocsPrintCalled = false; }")
    except Exception:
        pass
...
def is_zip(path: Path) -> bool:
    """Some Wealthfront tax forms (e.g. 1099-R) download as a ZIP holding the
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Wealthfront, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Wealthfront/order details",
