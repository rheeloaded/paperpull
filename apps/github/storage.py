"""What makes the GitHub app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is GitHub's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair GitHub's *page* behavior, edit `github_site.py` instead.
"""
...
SPEC = AppSpec(
    ...
    csv_files=[
        CsvSpec("order_history_csv", "GitHub Order History.csv", ORDER_HISTORY_COLUMNS),
        CsvSpec("receipt_index_csv", "GitHub Receipt Index.csv", RECEIPT_INDEX_COLUMNS),
    ],
    config_defaults={
        "pilot_online": 5,
    },
