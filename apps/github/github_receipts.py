"""GitHub purchase-history & receipt downloader (local, supervised).

Usage:
    python github_receipts.py --login
    python github_receipts.py --discover
    python github_receipts.py --pilot            (newest few orders)
    python github_receipts.py --all
    python github_receipts.py --resume
    python github_receipts.py --verify
    python github_receipts.py --review-names
    python github_receipts.py --diagnose [--order-number N]
    python github_receipts.py --dry-run
...
log = logging.getLogger("github_receipts")
...
class App:
    ...
    def browser(self):
        ...
        never scripted. Plain Chromium is enough for GitHub - it does not block
        ...
        if cdp_url:
            ...
            # window GitHub opens to print also has window.print() intercepted
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9264")
    ...
    def cmd_discover(self, types: Optional[List[str]] = None, quiet: bool = False) -> dict:
        """Discovery pass: the payment-history page, then ?page=2 and on
        until a page adds no payment the earlier pages did not have."""
        page = self.page()
        n_new = 0
        ...
        for page_no in range(1, 60):
            ...
            log.info("Payment history page %d: %d row(s), %d new", page_no, len(found), len(fresh))
            if not fresh:
                break
            for c in fresh:
        ...
        if not cards and site.history_state(page) != "empty":
            log.warning("No payment rows found. If you are signed in and do have "
                        "payments, run --diagnose and share the file.")
        for card in cards:
    ...
    def _select_purchases(self, ptype: Optional[str] = None,
        ...
        # (You already has 2024-and-earlier GitHub receipts).
    ...
    def process_one(self, page, purchase: Purchase, dry_run: bool = False):
        # The payment row already gave the date, amount and description.
    ...
    def _save_receipt(self, page, purchase: Purchase) -> bool:
        """Save the payment's receipt as a verified PDF.
        ...
        """
        if site.looks_signed_out(page):
            print("  GitHub is asking you to verify your sign-in.")
        ...
        if not url or not site.is_safe_url(url):
            self._record_state(purchase, State.NO_RECEIPT_AVAILABLE,
                               notes="The payment row carries no receipt link")
            ...
            self.stats["manual_review"] += 1
            print("  No receipt link on this payment's row - marked for manual review.")
            return False

        purchase.document_type = "Receipt"
        folder = self.paths.folder_for(purchase.purchase_type, purchase.document_type)
        filename = build_pdf_filename(purchase.purchase_date, purchase.summary, purchase.document_type)
    ...
    def _capture_document(self, target_page, purchase: Purchase,
        ...
        """Render the GitHub order-details receipt to PDF.

        GitHub ships NO print stylesheet, so printing the page as-is captures the
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='GitHub', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        """The payment-history page and what its first receipt link gives,
        written to Diagnostics/diagnose-github.json with every number of
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Local supervised GitHub receipt downloader")
    modes = [
        ("login", "open browser for manual GitHub sign-in"),
