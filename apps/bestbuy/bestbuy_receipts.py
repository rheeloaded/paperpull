"""Best Buy purchase-history & receipt downloader (local, supervised).

Usage:
    python bestbuy_receipts.py --login
    python bestbuy_receipts.py --discover
    python bestbuy_receipts.py --pilot            (newest few of each kind)
    python bestbuy_receipts.py --all
    python bestbuy_receipts.py --resume
    python bestbuy_receipts.py --verify
    python bestbuy_receipts.py --review-names
    python bestbuy_receipts.py --diagnose [--order-number N]
    python bestbuy_receipts.py --dry-run
...
log = logging.getLogger("bestbuy_receipts")
...
class App:
    ...
    def browser(self):
        ...
        never scripted. Best Buy sits behind bot protection, which is why the
        ...
        if cdp_url:
            ...
            # window Best Buy opens to print also has window.print() intercepted
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Best Buy appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9273")
    ...
    def cmd_discover(self, types: Optional[List[str]] = None, quiet: bool = False) -> dict:
        ...
        The history page shows a few purchases at once and draws only the
        ones on screen, and it opens on "Past 3 Years", which is capped. What
        it draws from is one query per year. So one year is chosen in its
        ...
        if references:
            log.info("%d older order(s) Best Buy keeps only as a reference, with no "
                     "details page, so there is no receipt to save", references)
        cards = [site.entry_to_purchase(e) for e in entries]
        dropped_by_scope = 0
        for purchase in cards:
            if not purchase:
                continue
    ...
    def _save_receipt(self, page, purchase: Purchase) -> bool:
        """Save the Best Buy details page as a verified PDF receipt.
        ...
        """
        if site.looks_signed_out(page):
            print("  Best Buy is asking you to verify your sign-in.")
    ...
    def _capture_document(self, target_page, purchase: Purchase,
                          out_path: Path) -> None:
        """Render the Best Buy details receipt to PDF.

        Printed as it stands the page runs to three pages of navigation and
        footer around the receipt. site.isolate_receipt keeps only the
        smallest visible block holding this purchase's number, a total and
        a SKU, a display change in the local page only, and CDP
        Page.printToPDF renders it. Print Receipt is never pressed.
        """
        site.isolate_receipt(target_page, purchase.order_number)
        try:
    ...
    def _pilot_report(self, selected: List[Purchase]):
        ...
        print("e.g.:  python bestbuy_receipts.py --all")

    def cmd_run(self, types: List[str], mode_name: str):
        self.stats["mode"] = mode_name
        if mode_name == "all" and not self.args.yes:
            print("This will download your FULL available Best Buy purchase history")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Best Buy', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        try:
            from datetime import date as _date
            site.goto_orders(page)
            offered = site.year_choices(page)
            query = site.capture_history_query(page, offered[0] if offered else str(_date.today().year))
            this_year = _date.today().year
            history = site.history_survey(page, query, range(this_year, this_year - 8, -1))
            history["years_offered"] = offered
            counted = [h for k, h in history.items() if k.isdigit()]
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Local supervised Best Buy receipt downloader")
    modes = [
        ("login", "open browser for manual Best Buy sign-in"),
