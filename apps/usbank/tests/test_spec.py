"""Locks in this app's identity - the values an existing archive depends on.

A renamed CSV orphans the index, a moved folder hides the documents, and a
different provider string renames every new PDF. Changing any of these is a
breaking change for anyone already running the app.
"""
import sys
...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-01-02", "thing", "Statement") ==         "2026-01-02 U.S. Bank Thing Statement.pdf"


def test_csv_filenames_are_unchanged(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.document_index_csv.name == "U.S. Bank Document Index.csv"
