"""U.S. Bank credit-card statement downloader (local, supervised).

Usage:
    python usbank_docs.py --login       verify connection to your browser
    python usbank_docs.py --discover    list available documents
    python usbank_docs.py --pilot       download the 5 newest, then stop
    python usbank_docs.py --all         download everything in scope
    python usbank_docs.py --resume      continue an interrupted run
    python usbank_docs.py --verify      re-validate every saved PDF
    python usbank_docs.py --diagnose    dump page structure (no downloads)
    python usbank_docs.py --dry-run     plan filenames, save nothing
...
U.S. Bank already generated. It never transfers funds, trades, rebalances,
...
log = logging.getLogger("usbank_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        # A per-document id, IF the page prints one. Chase's API supplied a
        # durable UUID; whether U.S. Bank exposes anything equivalent is
        # unconfirmed, so nothing here depends on it being present.
        self.document_id = document_id
        # `occurrence` is this document's position among a date's otherwise
        # indistinguishable set - the only handle the downloader has when a
        # provider labels several statements on one date identically.
    ...
    @property
    def key(self) -> str:
        """Stable identity. Use a per-document id when the page gives one,
        and fall back to category:date:title:account otherwise - which is what
        U.S. Bank is expected to need until a live probe shows better."""
        if self.document_id:
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's signed-in U.S. Bank tab (onlinebanking.usbank.com keeps the
            # session there; a fresh tab may be unauthenticated).
            live = [p for p in ctx.pages if not p.is_closed()]
            usbank = [p for p in live if "usbank.com" in (p.url or "")]
            self._work_page = usbank[0] if usbank else (live[0] if live else ctx.new_page())
        else:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! U.S. Bank appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in U.S. Bank browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but U.S. Bank shows a signed-out page.")
    ...
    def _record_usbank_doc(self, d: dict) -> int:
        ...
        if floor and (not date or date < floor):
            self.stats["skipped_out_of_scope"] += 1
            return 0
        # `account` is the card this document belongs to, as the page names
        # it ("Cash+ Visa Signature (...1234)"), and is empty on a single-card
        # login that never prints one. One card bills one statement per cycle,
        # so where a name IS printed it is unambiguous - it both re-finds the
        # row and goes in the filename.
        ...
        if account:
            ...
            # Two documents described identically on the same date. We will
            # not invent a difference between them: the filename carries a
            # traceable id fragment instead of a guessed card name.
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        if not site.ensure_statements(page):
            ...
            if not site.ensure_statements(page):
                print("Could not open your U.S. Bank statements. Sign in and open")
                print("Statements & Documents in the browser, then try again.")
                return 0
        self.check_session(page)

        # Two collectors, tried in order. The structured one drives the
        # page's own account and period pickers and reads each row; the
        # generic one scrapes whatever rows are currently rendered. Which of
        # them U.S. Bank needs is UNCONFIRMED - see the header of
        # usbank_site.py - so the fallback is not decoration here, it is the
        # safety net for a page shape this app has not yet seen.
        #
        # Reading a provider's own JSON API proved more reliable than scraping
        # on Ally and USAA, and --diagnose (probe_api) reports any endpoint
        # U.S. Bank calls for itself. Nothing here requests an endpoint that
        # has not been observed doing so.
        raw = site.usbank_collect_structured(page)
        if raw:
            log.info("U.S. Bank: %d document(s) read from the page's own pickers", len(raw))
        else:
            log.info("U.S. Bank: structured pass found nothing - falling back to scraping")
            print("  Could not read U.S. Bank's document pickers; reading the page instead.")
            raw = site.usbank_collect(page)
            log.info("U.S. Bank: collected %d document rows", len(raw))
        n_new = 0
        for d in raw:
            n_new += self._record_usbank_doc(d)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        The row is re-found by its accessible name (date, plus the card and
        action word when the page prints them) after the period and card
        pickers are set - never by a stored index, which shifts whenever a
        filter changes.
        """
        self.check_session(page)
        ...
        # Reach the statements page (reuse the signed-in tab); usbank_download
        # sets the period, selects the card and clicks the row's own document
        # link, keeping the browser's download event - or the PDF's bytes if
        # U.S. Bank opens it in a tab or serves it from a direct href instead.
        if not site.ensure_statements(page):
            self.check_session(page)
            site.ensure_statements(page)
        saved = site.usbank_download(page, page.context, doc.account, doc.date,
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available U.S. Bank documents ({scope}).")
    ...
    def cmd_diagnose(self):
        ...
        try:
            ...
            # The year filter is browser state and survives between runs, so
            # a report taken without this can describe an empty year.
            try:
                info["period_reset_to"] = site.reset_to_newest_period(page)
            except Exception as e:
                info["period_reset_to"] = f"ERR {e}"
            info["row_counts"] = {}
            ...
            # The generic scraper, kept as the redesign fallback...
            docs = site.collect_documents(page)
            info["collected_generic"] = len(docs)
            # ...and what the app ACTUALLY reads. On a working page the
            # generic count is 0 and this one is not: U.S. Bank renders no
            # table and no [role=row], so only the real reader sees the rows.
            try:
                info["account"] = site.account_name(page)
                info["statements_read"] = len(site.read_rows(page))
            except Exception as e:
                info["statements_read"] = f"ERR {e}"
            info["samples"] = []
            for d in docs[:8]:
            ...
            try:
                ...
                info["account_options"] = [re.sub(r"\$[\d,.]+", "$...", a)
                                           for a in accounts]
                # A picker need not be a <select>: Chase's year filter is a
                # styled <input>, and a card list can be accordion headers
                # instead. These report whichever shape U.S. Bank turns out
                # to use, so --diagnose answers that question directly.
                info["cards"] = [label for _el, label in site.card_groups(page)]
                info["period_options"] = site.period_options(page) or years
            except Exception as e:
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "U.S. Bank Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised U.S. Bank document downloader (read-only)")
