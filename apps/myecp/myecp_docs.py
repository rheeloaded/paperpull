"""MILITARY STAR card statement downloader (local, supervised).

Reads the Statements tab of the signed-in MyECP site (the Exchange Credit
Program), the same page fragment the tab loads, asked for from inside the
page, and saves every monthly statement PDF it lists. Nothing on the page
is clicked. The site layer, myecp_site.py, holds every fact about
myecp.com.

Usage:
    python myecp_docs.py --login       verify connection to your browser
    python myecp_docs.py --discover    list available documents
    python myecp_docs.py --pilot       download the 5 newest, then stop
    python myecp_docs.py --all         download everything in scope
    python myecp_docs.py --resume      continue an interrupted run
    python myecp_docs.py --verify      re-validate every saved PDF
    python myecp_docs.py --diagnose    dump page structure (no downloads)
    python myecp_docs.py --dry-run     plan filenames, save nothing
...
the Exchange already generated. It never pays, applies for credit, redeems
points, adds a user, or changes any account setting. Everything stays on this machine; nothing is sent to any
external service.
"""
...
log = logging.getLogger("myecp_docs")
...
class Document:
    ...
    @property
    def key(self) -> str:
        """Stable identity. Category, date, title and account. MyECP has
        no document id, and its StatementId is only a position that moves
        every month, so a statement is its card account plus its date."""
        if self.document_id:
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's already-signed-in MILITARY STAR tab.
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! MILITARY STAR appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9279")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in MILITARY STAR browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but MILITARY STAR shows a signed-out page.")
            print("Sign in in the open browser window (keep it OPEN), then re-run --login.")
        elif ok:
            print("Success: connected and signed in to MyECP.")
            print("Keep that browser window OPEN, then run:  paperpull myecp pilot")
        else:
            print("Connected, but I could not open the MyECP Account Summary.")
            print("Open www.myecp.com in that browser yourself, then run --diagnose.")
        self.close()
    ...
    def _record_rawdoc(self, r, source_url: str) -> int:
        ...
        # Empty with one card account, the account index with several.
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        page = self.page()
        n_new = 0
        # Be on the Account Summary, then ask each account's statements list.
        ...
        self.discovery.save()
        log.info("Statements: %d statements, %d new", len(docs), n_new)
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        try:
            ...
            print("\n  !! MILITARY STAR answered with a sign-in page instead of a statement.")
            print("     Your session has expired. Sign in again in the open")
            print("     browser, then run:  paperpull myecp resume")
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available MILITARY STAR documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='MILITARY STAR', text=text, extra=extra)
        if not path:
            return
    ...
    def write_survey(self) -> None:
        ...
        failure.write_survey(
            ...
            journal=self._journal,
            requests=self._requests,
            provider='MILITARY STAR')

    def cmd_diagnose(self):
        """Survey the summary page and what each statements list holds, and
        ...
        info = {"timestamp": now_iso(),
                "note": "What the MILITARY STAR statements list looks like, "
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "MILITARY STAR Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised MILITARY STAR document downloader (read-only)")
    ...
    ap.add_argument("--type", help="Statement (the only kind MyECP offers)")
