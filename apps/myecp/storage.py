"""What makes the MILITARY STAR app different, everything else is paperpull_core.
...
declaration of MILITARY STAR's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair MILITARY STAR's *page* behavior, edit `myecp_site.py` instead.
"""
...
# One row per downloaded MILITARY STAR document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # A credit card issues no tax forms, so the folder is made only if
