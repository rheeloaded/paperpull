"""What makes the Thrift Savings Plan app different. Everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is TSP's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair TSP's *page* behaviour, edit `tsp_site.py` instead.
"""
...
SPEC = AppSpec(
    provider="TSP",  # filenames say TSP, the folder says Thrift Savings Plan
    project_dir=Path(__file__).resolve().parent,
    kind=DOCUMENT,
    folders=[
        # Quarterly and annual participant statements share this folder,
        # told apart by their filename summary. Letters and notices from the
        # mailbox file here too, since the core has no letter category.
    ...
    csv_files=[
        CsvSpec("document_index_csv", "TSP Document Index.csv", DOCUMENT_INDEX_COLUMNS),
    ],
    config_defaults={
