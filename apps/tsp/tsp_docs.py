"""Thrift Savings Plan statement and tax-form downloader (local, supervised).

Usage:
    python tsp_docs.py --login       verify connection to your browser
    python tsp_docs.py --discover    list available documents
    python tsp_docs.py --pilot       download the 5 newest, then stop
    python tsp_docs.py --all         download everything in scope
    python tsp_docs.py --resume      continue an interrupted run
    python tsp_docs.py --verify      re-validate every saved PDF
    python tsp_docs.py --diagnose    dump page structure (no downloads)
    python tsp_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the statements and mailbox area and downloads
PDFs that TSP already generated. It never moves money between funds, changes
...
log = logging.getLogger("tsp_docs")
...
class App:
    ...
    def page(self):
        ...
        if self._cdp_mode:
            # Reuse the user's signed-in TSP tab; a fresh one is unauthenticated.
            # Matched by parsed host, not by substring: "tsp.com" in the URL
            # would also accept nottsp.com.example, and would have picked an
            # unrelated tab as the work page when no TSP tab was open.
            live = [p for p in ctx.pages if not p.is_closed()]
            mine = [p for p in live if site.is_safe_url(p.url or "")]
            if mine:
                self._work_page = mine[0]
            else:
                log.warning("No TSP tab is open. Sign in with login.bat and "
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! TSP appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in TSP browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but TSP shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has TSP documents from 2023 and earlier).
    ...
    def _write_row(self, doc: Document, status: str, processing: str):
        ...
        self.index_csv.append_rows([{
            ...
            # No URL exists for a TSP document: it is fetched by a
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available TSP documents ({scope}).")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised TSP document downloader (read-only)")
