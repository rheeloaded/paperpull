"""Fairfax Water bill downloader (local, supervised).

Usage:
    python fairfaxwater_docs.py --login       verify connection to your browser
    python fairfaxwater_docs.py --discover    list available documents
    python fairfaxwater_docs.py --pilot       download the 5 newest, then stop
    python fairfaxwater_docs.py --all         download everything in scope
    python fairfaxwater_docs.py --resume      continue an interrupted run
    python fairfaxwater_docs.py --verify      re-validate every saved PDF
    python fairfaxwater_docs.py --diagnose    dump page structure (no downloads)
    python fairfaxwater_docs.py --dry-run     plan filenames, save nothing

Filters: --year YYYY  --start-date YYYY-MM-DD  --end-date YYYY-MM-DD
         --max-docs N  --type Statement|"Tax Document"

READ-ONLY: this tool only reads the customer portal's bill history and downloads
PDFs that Fairfax Water already generated. It never moves money between funds, changes
...
Mapped 2026-09-18. See the STATUS block in fairfaxwater_site.py for the API, the
...
log = logging.getLogger("fairfaxwater_docs")
...
class Document:
    ...
    def __init__(self, title="", category="", summary="", date="", period="",
        ...
        self.item_id = item_id          # the row's bill date, a hint only, re-resolved
    ...
    @property
    def key(self) -> str:
        """Kind, account and date, never a portal id. The id is looked up fresh
        at download time. myPay's ids died with the session, and there is no
        reason to find out the hard way whether Fairfax Water's do."""
...
class App:
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Fairfax Water appears to have signed you out.")
    ...
    def cmd_login(self):
        print("Checking the connection to your signed-in Fairfax Water browser...\n")
        page = self.page()
        ...
        if challenge:
            ...
            print("Connected, but Fairfax Water shows a signed-out page.")
    ...
    def _in_scope(self, doc: Document) -> bool:
        ...
        # (You already has Fairfax Water documents from 2023 and earlier).
    ...
    def _record_statement_doc(self, d: dict, occurrence: int = 0) -> int:
        """Record one portal row {title, date, account, category, item_id, client_id}. Returns 1 if new."""
        ...
        if d.get("category"):
            # The portal says which kind a document is. Rules only choose the
        ...
        if acct_short.isupper():
            acct_short = acct_short.title()       # "BrokerageLink" keeps its own case
    ...
    def cmd_discover(self, quiet: bool = False) -> int:
        ...
        # share a title and a date (a bill and its correction, say), so
        ...
        raw = site.collect_documents(page, keep=scope.period_filter(self.args, self.config),
                                     config=self.config)
        log.info("Fairfax Water: %d bill(s) listed", len(raw))
        n_new = 0
        seen: dict = {}
        for d in raw:
    ...
    def download_one(self, page, doc: Document, filename: str):
        ...
        # Each PDF is what the row's View control opens in a new tab,
        # caught at the browser level. That is the one click this app
        # makes. The row is looked up again by title and date first.
        if not site.ensure_statements(page):
            self.check_session(page)
        ...
        try:
            ...
            print("\n  !! The portal answered with a sign-in error instead of a document.")
    ...
    def _write_row(self, doc: Document, status: str, processing: str):
        ...
        self.index_csv.append_rows([{
            ...
            # No URL exists for a Fairfax Water document: it is fetched by a
    ...
    def cmd_run(self, mode_name: str):
        ...
        if mode_name == "all" and not self.args.yes:
            ...
            print(f"This downloads ALL available Fairfax Water documents ({scope}).")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Fairfax Water', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        try:
            ...
            # fwcustomer.org pages only, the only links it follows are the few whose
            ...
            # No screenshot. An account page shows balances, and a
    ...
    def write_run_summary(self):
        ...
        atomic_write_text(self.paths.run_summary, "\n".join([
            "Fairfax Water Documents - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        description="Local supervised Fairfax Water document downloader (read-only)")
