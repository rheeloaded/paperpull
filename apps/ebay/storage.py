"""What makes the eBay app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is eBay's own
facts: the folders it files into, how a document routes to one, its CSV
columns, and its config defaults.

To repair eBay's *page* behavior, edit `ebay_site.py` instead.
"""
...
SPEC = AppSpec(
    ...
    csv_files=[
        CsvSpec("order_history_csv", "eBay Order History.csv", ORDER_HISTORY_COLUMNS),
        CsvSpec("receipt_index_csv", "eBay Receipt Index.csv", RECEIPT_INDEX_COLUMNS),
    ],
    config_defaults={
        "pilot_online": 5,
    },
