"""eBay purchase-history & receipt downloader (local, supervised).

Usage:
    python ebay_receipts.py --login
    python ebay_receipts.py --discover
    python ebay_receipts.py --pilot            (newest few orders)
    python ebay_receipts.py --all
    python ebay_receipts.py --resume
    python ebay_receipts.py --verify
    python ebay_receipts.py --review-names
    python ebay_receipts.py --diagnose [--order-number N]
    python ebay_receipts.py --dry-run
...
class DailyLimitReached(Exception):
    """eBay's daily cap on order-details pages, nothing more opens today."""
...
log = logging.getLogger("ebay_receipts")
...
class App:
    ...
    def browser(self):
        ...
        never scripted. Plain Chromium is enough for eBay - it does not block
        ...
        if cdp_url:
            ...
            # window eBay opens to print also has window.print() intercepted
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! eBay appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9262")
    ...
    # the three before it, so discovery walks the years back through the
    # filter's own vocabulary until two in a row come back empty, or the
    # vocabulary runs out. --year narrows the walk to that year.

    def _years_to_walk(self) -> List[int]:
        from datetime import date as _date
        this = _date.today().year
        if self.args.year:
            return [int(self.args.year)]
        floor = self.args.start_date or self.config.get("default_start_date")
        first = int(floor[:4]) if floor else this - len(site.YEAR_FILTER_WORDS) + 1
        return [y for y in range(this, first - 1, -1) if site.year_filter_word(y)]
    ...
    def cmd_discover(self, types: Optional[List[str]] = None, quiet: bool = False) -> dict:
        ...
        for year in self._years_to_walk():
            ...
            if found:
                ...
                empty_run += 1
                if empty_run >= 2 and not self.args.year:
                    log.info("two empty years in a row, the history ends here")
                    break
        if not cards:
    ...
    def _select_purchases(self, ptype: Optional[str] = None,
        ...
        # (You already has 2024-and-earlier eBay receipts).
    ...
    def process_purchases(self, purchases: List[Purchase], dry_run: bool = False):
        ...
        for i, purchase in enumerate(purchases, 1):
            ...
            try:
                ...
                self.progress.save(backup=True)
                print("\n!! eBay has reached its daily limit on order-details pages.")
                print("   Nothing more can be opened today. Progress is saved, and")
                print("   --resume continues tomorrow from this order.")
                self.stats["daily_limit"] = True
                break
            except Exception as e:
    ...
    def _save_receipt(self, page, purchase: Purchase) -> bool:
        """Save the eBay order-details page as a verified PDF receipt.

        eBay has no separate printable invoice: the hydrated order-details page
        ...
        """
        if site.looks_signed_out(page):
            print("  eBay is asking you to verify your sign-in.")
        ...
        # Make sure we are on this order's details page (the receipt). An
        # older id carries a "!" that the address bar shows as %21.
        if not site.on_details_page(page) or purchase.order_number not in unquote(page.url or ""):
            site.goto_details(page, purchase)
        site.scroll_full_page(page)

        card_only = False
        if not site.receipt_is_present(page) and site.order_not_found(page):
            # eBay has lost the details page for some orders from 2019 and
            # before, and says "Order not found" for them every time. The
            # purchase history still lists the order, so its card, with
            # the date, item, total, seller and order number, is printed
        ...
        try:
            self._capture_document(page, purchase, out_path,
                                   content_kind="card" if card_only else "")
    ...
    def _capture_document(self, target_page, purchase: Purchase,
        ...
        """Render the eBay order-details receipt to PDF.

        eBay ships NO print stylesheet, so printing the page as-is captures the
        ...
        Page.printToPDF.
        """
        # Strip the site chrome so only the receipt is printed. A history
        # card has already been isolated by show_order_card.
    ...
    def write_run_summary(self):
        ...
        if s.get("daily_limit"):
            lines.insert(-1, "Stopped at eBay's daily limit on order-details pages. Run --resume tomorrow.")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Local supervised eBay receipt downloader")
    modes = [
        ("login", "open browser for manual eBay sign-in"),
