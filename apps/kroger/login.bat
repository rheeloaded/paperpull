@echo off
cd /d "%~dp0"
rem  login.bat          -> your account (config.json, port 9263)
rem  login.bat spouse   -> config.spouse.json (own folders, profile, port)
if "%~1"=="" (set "CFG=") else (set "CFG=--config config.%~1.json")
echo ============================================================
echo  Kroger Receipts - sign in
echo ============================================================
if not "%~1"=="" echo Account: %~1
echo.
echo A browser window will open. Then:
echo   1. Sign in to your Kroger account (handle any OTP / puzzle yourself).
...
echo.
.venv\Scripts\python.exe kroger_receipts.py --open-browser %CFG%
echo.
pause
