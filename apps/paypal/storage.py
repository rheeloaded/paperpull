"""What makes the PayPal app different, everything else is paperpull_core.
...
declaration of PayPal's own facts: the folders it files into, how a document
routes to one, its CSV columns, and its config defaults.

To repair PayPal's *page* behavior, edit `paypal_site.py` instead.
"""
...
# One row per downloaded PayPal document. Deliberately records NO account
...
SPEC = AppSpec(
    ...
    folders=[
        ...
        # Tax forms are not saved yet, so the folder is made only if a
        # title ever routes there, never left sitting empty.
