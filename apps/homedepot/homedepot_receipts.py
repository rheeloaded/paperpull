"""Home Depot purchase-history & receipt downloader (local, supervised).

Usage:
    python homedepot_receipts.py --login
    python homedepot_receipts.py --discover
    python homedepot_receipts.py --pilot            (newest few of each kind)
    python homedepot_receipts.py --all
    python homedepot_receipts.py --resume
    python homedepot_receipts.py --verify
    python homedepot_receipts.py --review-names
    python homedepot_receipts.py --diagnose [--order-number N]
    python homedepot_receipts.py --dry-run
...
log = logging.getLogger("homedepot_receipts")
...
class App:
    ...
    def browser(self):
        ...
        never scripted. Home Depot sits behind bot protection, which is why the
        ...
        if cdp_url:
            ...
            # window Home Depot opens to print also has window.print() intercepted
    ...
    def check_session(self, page) -> None:
        ...
        if site.looks_signed_out(page):
            self.progress.save(backup=True)
            print("\n!! Home Depot appears to have signed you out.")
    ...
    def cmd_open_browser(self):
        ...
        """
        port = browser_launcher.port_from_cdp_url(self.config.get("cdp_url", ""), "9272")
    ...
    def cmd_discover(self, types: Optional[List[str]] = None, quiet: bool = False) -> dict:
        ...
        POST. That request, its address and its date range, is taken as the
        ...
        if not quiet:
            ...
            print("  months and nothing is lost.")
            if dropped_by_scope:
    ...
    def _save_receipt(self, page, purchase: Purchase) -> bool:
        """Save the Home Depot receipt as a verified PDF.
        ...
        """
        if site.looks_signed_out(page):
            print("  Home Depot is asking you to verify your sign-in.")
    ...
    def _capture_document(self, target_page, purchase: Purchase,
                          out_path: Path) -> None:
        """Render Home Depot's own receipt to PDF.

        The receipt is already on the details page, shown only in print. The
        page is switched to print media, everything but the receipt is
        hidden, and it is printed with CDP Page.printToPDF. View Receipt,
        which only opens the print dialog, is never pressed. The page goes
        back to screen media afterwards either way.
        """
        site.isolate_receipt(target_page)
        try:
            log.info("Capture path: print-only receipt block printToPDF")
            receipt_pdf.print_page_to_pdf(target_page, out_path)
            return
        except Exception as e:
            log.warning("Live printToPDF failed (%s); trying fallbacks", e)
        finally:
            site.restore_screen(target_page)
    ...
    def _pilot_report(self, selected: List[Purchase]):
        ...
        print("e.g.:  python homedepot_receipts.py --all")

    def cmd_run(self, types: List[str], mode_name: str):
        self.stats["mode"] = mode_name
        if mode_name == "all" and not self.args.yes:
            print("This will download your FULL available Home Depot purchase history")
    ...
    def write_failure(self, step: str, reason: str, text: str = "",
        ...
        path = failure.write_failure(
            ...
            provider='Home Depot', text=text, extra=extra)
        if not path:
            return
    ...
    def cmd_diagnose(self):
        ...
        try:
            request = site.goto_orders(page)
            history = site.history_survey(page, request)
            after = {"cards_collected": history.get("orders"),
                     "became_purchases": history.get("orders")}
    ...
    def write_run_summary(self):
        ...
        lines = [
            "Home Depot Receipts - run summary",
            "=" * 40,
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Local supervised Home Depot receipt downloader")
    modes = [
        ("login", "open browser for manual Home Depot sign-in"),
