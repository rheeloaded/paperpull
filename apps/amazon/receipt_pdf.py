...
log = logging.getLogger("amazon_receipts.pdf")
...
(() => {
  try {
    ...
    window.__amazonReceiptsOriginalPrint = orig;
    window.__amazonReceiptsPrintCalled = false;
    window.print = function () {
      window.__amazonReceiptsPrintCalled = true;
      ...
      try {
        ...
        window.__amazonReceiptsPrintHTML = html;
        try {
          window.top.__amazonReceiptsPrintHTML = html;
          window.top.__amazonReceiptsPrintCalled = true;
        } catch (e) {}
      } catch (e) {}
    };
  } catch (e) {}
...
(() => {
  try {
    if (window.__amazonReceiptsOriginalPrint) {
      window.print = window.__amazonReceiptsOriginalPrint;
    }
  } catch (e) {}
})();
"""
...
# the receipts column on pages without stable ids/landmarks (Amazon's
...
(needles) => {
  ...
  best.setAttribute('data-amazon-receipts-isolate', '1');
  return true;
}
"""
...
() => {
  ...
  let hidden = 0;
  const root = document.querySelector('[data-amazon-receipts-isolate]')
...
def isolate_online_receipts(page) -> None:
    ...
    try:
        ...
        pass
    if found:
        isolate_for_print(page, "[data-amazon-receipts-isolate='1']")
    else:
...
def was_print_called(page) -> bool:
    try:
        return bool(page.evaluate("() => window.__amazonReceiptsPrintCalled === true"))
    except Exception:
        return False
...
def print_html_to_pdf(page, html: str, out_path: Path) -> None:
    ...
    if "<base" not in html.lower():
        html = re.sub(r"(<head[^>]*>)",
                      lambda m: m.group(1) + '<base href="https://www.amazon.com/">',
...
def get_print_snapshot(page) -> Optional[str]:
    ...
    try:
        html = page.evaluate("() => window.__amazonReceiptsPrintHTML || null")
...
def clear_print_snapshot(page) -> None:
    try:
        page.evaluate("() => { window.__amazonReceiptsPrintHTML = null; "
                      "window.__amazonReceiptsPrintCalled = false; }")
    except Exception:
        pass
...
def validate_pdf(path: Path, min_bytes: int = 3000,
    ...
    tokens (Amazon, date, order number, item names). An image-based PDF with
        ...
        try:
            ...
            if text_lower.strip() and not token_found:
                ...
                return ValidationResult(
                    False, "Extractable text does not mention Amazon/order details",
