...
def test_provider_string_is_unchanged():
    ...
    assert storage.build_pdf_filename("2026-01-02", "thing", "Statement") ==         "2026-01-02 Amazon Thing Statement.pdf"


def test_csv_filenames_are_unchanged(tmp_path):
    paths = storage.Paths(tmp_path)
    assert paths.order_history_csv.name == "Amazon Order History.csv"
    assert paths.receipt_index_csv.name == "Amazon Receipt Index.csv"
