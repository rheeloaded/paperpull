"""What makes the Amazon app different - everything else is paperpull_core.

This file used to be a full copy of the storage logic every other app carried.
That logic now lives in `paperpull_core`; what remains here is Amazon's own
...
SPEC = AppSpec(
    ...
    csv_files=[
        CsvSpec("order_history_csv", "Amazon Order History.csv", ORDER_HISTORY_COLUMNS),
        CsvSpec("receipt_index_csv", "Amazon Receipt Index.csv", RECEIPT_INDEX_COLUMNS),
    ],
    config_defaults={
        "pilot_online": 5,
    },
